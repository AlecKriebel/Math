...
**Published on Zenodo:** [10.5281/zenodo.22983519](https://doi.org/10.5281/zenodo.22983519). [Publication and tracker receipt](publication/README.md). The v1.0.1 PDF and upload archives are the unchanged deposited snapshot.
...
To reproduce the deposited package, use the source extracted from its published archive. The current working tree includes later DOI and publication notices; rebuilding it prepares different archive bytes. Alternatively compile the source twice with a conventional LaTeX distribution. `build_package.py` creates deterministic ZIP archives, payload checksums, and the self-contained web page's download files. It does not publish, upload, or contact anyone. Tectonic needs its usual TeX package cache or network access on the first build; the delivered PDF is ready to read.
...
This is an unrefereed, AI-assisted preprint, prepared with OpenAI Codex and separate adversarial AI reviewers from the supplied AI-generated candidate. No external human peer review is claimed. The Zenodo DOI identifies the published preprint; it does not establish historical priority. The two supplied gradient-path filenames belong to a different problem and are not used here.
