...
\date{26 September 2026\quad (version 1.0.1)}
\begin{document}
\maketitle
\begin{abstract}
...
three. The proof is independent of computation. Historical priority is
unresolved; no claim of a first resolution is made.
\end{abstract}
...
\end{equation}
This is the standard statistical $(-1)$-conformal change printed in
\cite[p.~178, equation (2.1)]{Matsuzoe}, with conformal factor $t$.
It combines a projective change of connection with a conformal metric
change. We use it only to remove the possible restriction to self-dual
examples; the deformation itself is prior work.
...
This note supplies an explicit negative answer to the printed Question
3(e), with a short direct verification. We do not claim that the Gauss
lemma, the curvature criterion, or the construction in \eqref{extension}
is new.

\textbf{Historical priority remains unresolved.} The documented,
bounded literature search found no earlier explicit answer, but relevant
sources remain uninspected: Kurose's 1999 affine-realization paper,
Binder--Simon's 2000 problem list, and the full content of Kurose's 2016
talk on Gauss-type lemmas and Hessian structures, among other citation leads.
An earlier answer or implicit resolution in those sources cannot be
excluded. Accordingly, this preprint makes no first-resolution claim and
does not assert that the question remained open at publication. The live
database annotations were subsequently accessed; their machine-generated
report is not evidence establishing priority. The accompanying dated
audits record search coverage and remaining gaps separately from the proof.
...
\bibitem{Matsuzoe}
H.~Matsuzoe, Geometry of contrast functions and conformal geometry,
\emph{Hiroshima Mathematical Journal} \textbf{29} (1999), 175--191.
\href{https://doi.org/10.32917/hmj/1206125160}{doi:10.32917/hmj/1206125160}.
\end{thebibliography}
\end{document}
