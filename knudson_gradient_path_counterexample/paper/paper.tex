...
\hypersetup{
  pdftitle={A four-vertex counterexample to Knudson's persistence-gradient conjecture},
  pdfauthor={Alec Kriebel},
  pdfsubject={A counterexample to Conjecture 2 in Oberwolfach Report 29/2008},
  pdfkeywords={persistent homology, discrete Morse theory, gradient paths, counterexample}
}
...
\date{23 September 2026 \quad (version 1.0.1)}
\begin{document}
\maketitle
\begin{abstract}
...
simplex to a facet, and matched incidences point upward. A simplex is critical
if it belongs to no matching pair. A gradient path from a critical edge begins
by descending to an endpoint. Each subsequent upward step uses a matched
incidence and is followed by descent to the other endpoint of that edge.
...
The indices $1,\ldots,7$ order the filtration; the field $V_P$ is constructed
from the persistence pairs.
...
The \href{https://aleckriebel.github.io/Math/papers/knudson-gradient-path/}{companion package and audit record}
provide two standard-library Python verifiers:
...
search, not a guarantee of priority; in particular, an
\href{https://kpknudson.com/math}{author bibliography}
