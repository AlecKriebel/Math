...
# Exact directed core templates reused from the certified JC/K3P topology universe.
# Algebra below is regenerated for all three independent K3P character sectors.
CORES = {
    'cycle': {
        'arcs': (('S','X'),('S','X')),
        'retics': ('X',),
...
def orbit_assignments(k:int):
    """K3P C/T-orbit assignments retained only for regression checks."""
    return tuple(sorted({ct_orbit_rep(c) for c in zero_sum_assignments(k)}))

@lru_cache(maxsize=None)
def k3p_assignments(k:int):
    """All conservation-supported K3P Fourier assignments."""
    return tuple(zero_sum_assignments(k))
...
def sector_for_mask(mask:int,chars):
    """Return exact character 0,C,G,T encoded as 0,1,2,3."""
    x=0;i=0
    while mask:
        if mask&1:x^=chars[i]
        i+=1;mask>>=1
...
def eval_descriptor(desc:MapDescriptor, edge_triples, lambdas):
    ...
    for expr in desc.outputs:
        ...
        for mon,poly in expr:
            mval=Fraction(1)
            for ci,sec,exp in mon:mval*=edge_triples[ci][sec-1]**exp
            pval=Fraction(0)
...
def default_exact_point(desc:MapDescriptor, salt:int=0):
    """Deterministic strict continuous-time K3P point."""
    from fractions import Fraction as F
    edges=[]
    for i in range(desc.edge_class_count):
        c=F(3*i+5+salt, 12*i+23+4*salt)
        g=F(5*i+7+salt, 18*i+29+3*salt)
        t=F(7*i+9+salt, 22*i+35+2*salt)
        assert 0<c<F(1,3) and 0<g<F(1,3) and 0<t<F(1,3)
        assert 1+c-g-t>0 and 1-c+g-t>0 and 1-c-g+t>0
        assert c>g*t and g>c*t and t>c*g
        edges.append((c,g,t))
    lams=[]
...
def descriptor_jacobian(desc:MapDescriptor, edge_triples=None,lambdas=None):
    from fractions import Fraction as F
    if edge_triples is None:edge_triples,lambdas=default_exact_point(desc)
    assert lambdas is not None
    p=3*desc.edge_class_count+desc.retic_count
    rows=[]
    for expr in desc.outputs:
        ...
        for mon,poly in expr:
            ...
            for ci,sec,exp in mon:
                var=3*ci+(sec-1);powers[var]=exp;mval*=edge_triples[ci][sec-1]**exp
            ...
            for var,exp in powers.items():
                ci=var//3;sec=var%3
                grad[var]+=mval*pval*exp/edge_triples[ci][sec]
            for j,x in enumerate(pd):grad[3*desc.edge_class_count+j]+=mval*x
        rows.append(grad)
    return rows
...
def rank_certificate(desc:MapDescriptor,salt=0):
    ...
    assert (rank==0)==(det==0)
    return {'rank':rank,'rows':rows,'columns':cols,'determinant':str(det),'edge_triples':tuple(tuple(map(str,e)) for e in edges),'lambdas':tuple(map(str,lams))}
...
def output_sparse_polynomials(desc:MapDescriptor):
    ...
    Exponent tuple order: c_0,g_0,t_0,...,c_E,g_E,t_E,lambda_0,... .
    """
    p=3*desc.edge_class_count+desc.retic_count
    out=[]
    for expr in desc.outputs:
        ...
        for mon,lpoly in expr:
            base=[0]*p
            for ci,sec,exp in mon:base[3*ci+sec-1]+=exp
            for mask,coef in lpoly:
                ex=list(base)
                for j in range(desc.retic_count):
                    if mask>>j&1:ex[3*desc.edge_class_count+j]+=1
                poly[tuple(ex)]+=coef
...
def coordinate_weights(k:int):
    """Three incidence degrees per port, one for each fixed nonzero label."""
    ans=[]
    for chars in k3p_assignments(k):
        row=[]
        for x in chars: row.extend((x==1,x==2,x==3))
        ans.append(tuple(map(int,row)))
    return tuple(ans)
...
def exact_kernel_sparse_columns(columns):
    ...
    for j,col in enumerate(columns):
        for e,c in col.items():M[rid[e],j]=sp.Integer(c)
    ns=M.nullspace()
