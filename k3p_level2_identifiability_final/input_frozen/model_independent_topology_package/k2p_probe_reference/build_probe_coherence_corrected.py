#!/usr/bin/env python3
"""Build the corrected all-primitive K2P one-/two-port probe closure.
...
3. direct full-map T_i zero/strict-sign pullback;
4. unresolved (fatal).
...
INPUT_MUTATIONS = PROJECT / "work/adversarial_proof_review/probe_input_mutation_certificate.json"
...
OUTPUT = HERE / "probe_coherence_certificate.json"
...
def classify_probe_pair(
    ...
    if sign_id is not None:
        return {
            "status": "full_map_Ti_strict_sign",
            "proof_id": sign_id,
        }, None
    return {
        "status": "unresolved",
...
def seal_proof_registry(proof_registry, sign_oracle):
    registries = {
        "schema": "k2p-corrected-probe-separation-registries-v1",
        "separation_proof_registry": dict(sorted(proof_registry.items())),
        "full_map_Ti_registry": sign_oracle.public(),
    }
...
def base_certificate(
    ...
    del restoration
    return {
        "schema": "k2p-corrected-coherent-probe-closure-v1",
        "status": "IN_PROGRESS",
        "claim_boundary": (
            "Exact relation first, displayed quartet second, and direct original-full-map "
            "T_i/Bernstein strict sign third. No rooted triple restriction is consulted."
        ),
        "inputs": {
        ...
        "registries": {
            "separation": {
                ...
                "full_map_Ti_relation_certificates": len(
                    proof_registries["full_map_Ti_registry"]["certificates"]
                ),
                "full_map_Ti_strict_polynomials": len(
                    proof_registries["full_map_Ti_registry"]["strict_polynomial_registry"]
                ),
            },
...
def main():
    ...
    checkpoint["assembly_theorem"] = {
        ...
        "bridge_torus_compatibility": {
            "every_Ti_certificate_boundary_multihomogeneous": True,
            "sector_weights": "two selected non-i arms s^2*g; selected i arm g^2; all other arms zero",
            "conclusion": "all sign separators survive the complete physical two-sector incidence action",
        },
        "unresolved": 0,
        "incoherent": 0,
        "conclusion": (
