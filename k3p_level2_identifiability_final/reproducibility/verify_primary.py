...
def corrected_transport_replay():
    ...
    if not audit_path.is_file():
        return {"status": "BLOCKED", "gap": "corrected root-suppressed mixed-graph transport audit is absent"}
    ...
    required = (
        audit.get("schema") == "k3p-h21-01-transport-audit-v1"
        and audit.get("status") == "PASS"
        and audit.get("primary_transport_status") == "VALID"
        and audit.get("exact_remaining_gaps") == []
        and audit.get("full_replay", {}).get("canonical_orbits") == 14
        and audit.get("full_replay", {}).get("raw_orbit_members") == 38
        and audit.get("full_replay", {}).get("prelock_sink_swaps") == 2
        and audit.get("full_replay", {}).get("all_exact_certificate_replays_pass") is True
        and audit.get("full_replay", {}).get("coverage_zero_unresolved") is True
        and replay["status"] == "PASS"
    )
    expected_script_hash = audit.get("corrected_verifier", {}).get("sha256")
    ...
    replay.update(
        {
            ...
            "audit_primary_transport_status": audit.get("primary_transport_status"),
            "audit_exact_remaining_gaps": audit.get("exact_remaining_gaps"),
            "full_replay": audit.get("full_replay"),
        }
    )
    return replay
...
def stable_replay_record(record):
    if record is None:
        return None
    # Timing belongs in the ignored per-run transcript.  Omitting it here
    # keeps the tracked mathematical gate report byte-identical on clean runs.
...
def main():
    ...
    if input_binding["status"] != "PASS":
        ...
        cut = exact_family("cut_recovery", cut_recovery_evidence, FROZEN)
        ...
        outputs = [
            ...
            ("cut_recovery/primary_exact_evidence.json", cut),
        ...
        if cut and cut["status"] == "PASS":
            gates.append(make_gate(6, "PASS", ["cut_recovery/primary_exact_evidence.json"]))
        else:
            gates.append(make_gate(6, "BLOCKED" if cut else "FAIL", ["cut_recovery/primary_exact_evidence.json"], cut.get("exact_gap") if cut else family_errors.get("cut_recovery")))
        ...
        for number in (19,):
            gates.append(make_gate(number, transport_status, ["four_port_atlas/primary_exact_evidence.json", "clean_room/H21_01_TRANSPORT_AUDIT.json"], transport.get("gap") if transport_status != "PASS" else None, "The project-local primary implementation reconstructs all mixed automorphism groups, double cosets, and literal Fourier coordinate actions. The corrected clean-room implementation is an additional cross-check, not a substitute."))
        ...
        gates.append(make_gate(23, "PASS" if transport_status == "PASS" and four and four["accounting_numerically_consistent"] and four["accounting_classification_certified_by_this_module"] else ("BLOCKED" if transport_status == "BLOCKED" else "FAIL"), ["four_port_atlas/primary_exact_evidence.json", "clean_room/H21_01_TRANSPORT_AUDIT.json"], transport.get("gap") if transport_status != "PASS" else None))
