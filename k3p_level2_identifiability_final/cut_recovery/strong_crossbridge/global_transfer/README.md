...
the direct verifier checks the exact typed nine-row declaration and rejects
48/48 targeted mutations, including one coherent claim-body mutation per row.
...
closure, and rejects 44/44 targeted mutations. `verify_release.py` checks the
sealed layers without importing either verifier and freshly executes both
semantic verifiers; `THEOREM_MANIFEST.json` binds their reports and manifests.
