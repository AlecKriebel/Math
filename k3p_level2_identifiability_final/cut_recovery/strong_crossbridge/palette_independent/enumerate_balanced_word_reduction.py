...
* some segment contains at least three monochromatic runs, which is the
  direct path-order obstruction proved in the balanced-compression lemma of
  ``manuscript/sections/04_physical_topology.tex``; or
* collapsing every run to one representative gives the short palette used by
  ``verify_reduced_palette_cleanroom.py``, possibly followed by the one
  permitted adjacent duplicate when a colour would otherwise have only one
  representative.

Thus this file verifies the combinatorial exhaustiveness of the reduction;
``verify_reduced_palette_cleanroom.py`` independently verifies that the
resulting finite palette has no all-switching survivor.
"""
