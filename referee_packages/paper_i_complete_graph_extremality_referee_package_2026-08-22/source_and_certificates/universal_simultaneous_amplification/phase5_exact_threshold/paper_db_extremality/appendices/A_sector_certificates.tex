\section{Hessian-sector certificates for every population size}\label{app:sectors}
...
\]
The table in Section~\ref{sec:hessian} instead uses Frobenius normalization
on the embedded matrix.  Direct summation of \eqref{4.8} gives
\[
 \|E(\xi)\|_F^2
 =\frac{n-1}{n(n-2)}\|\xi\|^2,                               \tag{A.12n}\label{A.12n}
\]
and therefore the Frobenius-normalized standard eigenvalue is
$\Phi_N/[4N(N+1)]$.  For example, the canonical $n=3$ ratio
$\Rtwo_3(E(\xi))/\|\xi\|^2=2/33$ becomes the table value $1/11$.
...
\]
This proves the standard-sector sign for every population size.  The supplementary
...
\]
\begin{lemma}[strict rank-Poisson gradients]\label{lem:rank-gradient}
...
\begin{proof}
Realize the chain \eqref{A.2} on $N-1$ Boolean coordinates.  At each step
choose uniformly among these coordinates and one dummy slot; a genuine
coordinate, when chosen, is refreshed to an independent fair bit, whereas
the dummy leaves the configuration unchanged.  If $K_t$ is one plus the
number of occupied coordinates, its transition probabilities are exactly
\eqref{A.2}.  For $f(k)=k^{-1}-c_0$, stationarity gives $\pi f=0$, and finite
ergodicity gives the convergent Poisson representation, modulo an irrelevant
constant,
\[
 h_k=\sum_{t\ge0}\E_k f(K_t).
\]

Couple configurations of ranks $k$ and $k+1$ that differ only at a
distinguished bit $a$, using the same chosen slots and fair refreshes.  Until
the first refresh of $a$, write $X_t$ for the number of occupied common
coordinates.  The forcing difference is
\[
 g(X_t):=\frac1{X_t+1}-\frac1{X_t+2}>0;
\]
after that refresh the two chains coalesce.  Termwise summation of the
Poisson series therefore gives $d_k>0$.

For $1\le k\le N-2$, choose another bit $b$ and a set $S$ of $k-1$
occupied bits, disjoint from $a,b$.  Start four chains from
\[
 S,\qquad S\cup\{a\},\qquad S\cup\{b\},\qquad S\cup\{a,b\},
\]
and again use identical refreshes.  The first pair represents $d_k$ and the
second represents $d_{k+1}$.  The common configuration of the second pair
contains that of the first until $b$ is refreshed, after which the
corresponding common configurations coincide.  Since $g$ is strictly
decreasing, every summand for $d_k$ dominates the corresponding summand for
$d_{k+1}$, and the inequality is strict already at time zero.  Hence
$d_k>d_{k+1}$, proving the lemma.
\end{proof}
...
\[
 ...
 z(B)=\sum_{\substack{w,i\in B\\w\ne i}}\delta_{wi}.       \tag{A.11}\label{A.11}
\]
...
\]
Here is the labelled normalization valid for every $n$.  Let
...
$g^{\mathsf T}(a,b)=g^{\mathsf T}(I-K)^{-1}s$.  This is a labelled
...
\]
\begin{lemma}[finite exact symmetric-sector certificate]
\label{lem:symmetric-finite}
The scalar $\mathcal S_N$ is positive for $3\le N\le39$.  For each of the
248 integer orders $40\le N\le287$,
$\beta_N+\varepsilon_N<1$; the smallest certified margin occurs at $N=40$
and is
\[
...
The included exact-arithmetic program
\begin{center}
\small\path{verify_true_inverse_rank_symmetric_phase.py}
\end{center}
directly solves the rational systems for $3\le N\le39$, recursively constructs the rational
$t_j$, $\beta_N$, and $\varepsilon_N$ for all 248 remaining finite orders,
asserts every sign and the minimum in \eqref{A.33}, and uses no floating-point
...
\begin{center}
\small\texttt{PASS: stationary symmetric inverse-rank phase certificate}
\end{center}
and prints
the three ranges $3\!:\!39$, $40\!:\!287$, and $N\ge288$ separately.  The
deterministic source bundle contains this exact file and its dependencies.
\end{proof}
...
$\beta_N<19/20$ follows, after clearing positive factors, from
$G_N(j)>0$, where
\[
\begin{aligned}
...
rank.  Also $\varepsilon_N<1/20$ for $N\ge46$: after clearing its positive
denominator this is $22N^2-1066N+2555>0$, which holds at $N=46$ and is
increasing thereafter.  Therefore
...
exactly over $\mathbb Q$ by Lemma~\ref{lem:symmetric-finite}; the first values are
