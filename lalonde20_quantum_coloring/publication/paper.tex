...
witnesses.  In fact, Lalonde's fixed-dimension and fixed-rank parameters also
satisfy $\chi_q^{[d]}(J_n)=\chi_q^{(s)}(J_n)=n+1$ for every $d,s\geq1$.
...
Here $\xi$ denotes complex orthogonal rank.
Thus the graph in the question is $H=J_4$, with vertex 20 as the joined
apex.
...
chromatic number restricted to local dimension $d$, and $\chi_q^{(r)}$ for
the rank-$r$ quantum chromatic number.
...
fixed-rank upper bound.  Section 1's orthogonal-rank calculation completes the
theorem.
\end{proof}
...
compressions, $J$ intertwining, dimension count, and an independent trace-SOS
formulation with clique and cross-color checks.  A separate graph checker
