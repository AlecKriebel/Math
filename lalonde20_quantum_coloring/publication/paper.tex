...
Lalonde and put $J_n=G_{19}\join K_{n-3}$ for $n\geq3$.  Lalonde proved the
claimed value for color projectors of rank one~\cite[Theorem~1.5]{Lalonde2026}
and conjectured the same value without a rank restriction
\cite[discussion following Theorem~1.5]{Lalonde2026}.  We prove that
conjecture in the finite-dimensional projector model:
\[
...
representations, and irreducible representations alike.  The induced subgraph
on vertices $1,\ldots,13$ is the Man\v{c}inska--Roberson graph $G_{13}$.  Their
operator propagation proving $\chi_q(G_{13})=4$~\cite{MancinskaRoberson2016}
is therefore a relevant earlier higher-rank rigidity precedent; the SOS and
exhaustive invariant-plane tail classification here use the additional
structure of the present 19-vertex graph.

The short verifier \nolinkurl{verify_obstruction_certificate.py} reads
\nolinkurl{obstruction_certificate.json} and replays the paper's four-factor
SOS, neighbor signs, six tail compressions, $J$ intertwining, and terminal
dimension coefficients.  Independently,
\nolinkurl{verify_lalonde_uniform_obstruction.py} reads
\nolinkurl{lalonde_uniform_obstruction.json}, uses a distinct trace-SOS
formulation, and additionally checks graph6 decoding, the clique-ideal
witness, and row reduction of the cross-color kernel.  A separate
\nolinkurl{verify_graph.py} checks the canonical graph data.  All arithmetic
is exact and rational; no numerical SDP output enters the theorem.

\paragraph{Artifact provenance.}
The exact algebraic artifacts audited before these presentation revisions are
frozen in an
...
\end{quote}
The final source commit and rendered-PDF digest are release metadata recorded
after typesetting is frozen; omitting them here avoids a self-referential
artifact hash.
...
\emph{Oddities of quantum colorings},
Baltic J. Modern Computing \textbf{4} (2016), no.~4, 846--859,
\href{https://doi.org/10.22364/bjmc.2016.4.4.16}{doi:10.22364/bjmc.2016.4.4.16}.
\end{thebibliography}
