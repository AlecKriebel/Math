...
therefore rules out a family required to amplify at every fitness for each
sufficiently large member.  It does not settle the reversed asymptotic order
in which the size threshold may depend on a fixed fitness value.
\end{abstract}
...
for every $r$.  In particular, no family $(G_N)$ can satisfy
\[
 \exists N_0\ \forall N\ge N_0\ \forall r>1:\quad
 \rho_{\dB}(G_N,r)>\rho_{\dB}(K_{|V(G_N)|},r).
\]
Thus simultaneous Bd/dB amplification with this order of quantifiers is
impossible.  The theorem does not decide whether
\[
 \forall r>1\ \exists N_0(r)\ \forall N\ge N_0(r)
\]
both strict amplification inequalities can hold for one family.
\end{theorem}
...
formula.  Define
\[
 R_i=\sum_{j\ne i}a_{ij},\qquad C_i=\sum_{j\ne i}a_{ji}.
\]
Since
$\sum_{j\ne i}b_{ij}=(R_i+C_i)/(n(n-2))$, expansion of the preceding
singleton equation gives
\begin{equation}\label{eq:singleton-expansion}
 q_{\{i\}}(\varepsilon)
 =\frac1n+\frac{\varepsilon}{n^2}
 \left[C_i+\frac{R_i+C_i}{n-2}\right]+O(\varepsilon^2).
\end{equation}
Now $\sum_iR_i=\sum_iC_i=T(G)$, and averaging
...
absorbing equations, and extracts the limiting coefficients in representative
finite instances.  These finite computations validate the implementation and
replay benchmark consequences; they do not discharge the theorem's universal
quantifier.  That quantifier is proved by the symbolic identities
\eqref{eq:support-gap}, \eqref{eq:differentiated-system}--
\eqref{eq:singleton-expansion}, and \eqref{eq:sos}.
