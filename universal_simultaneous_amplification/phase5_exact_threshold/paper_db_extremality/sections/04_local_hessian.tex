...
\]
All entries are analytic for $\varepsilon$ near zero, and $K_0$ is
irreducible.  Put
\[
 q=H-c_0\one,\qquad
...
All large orders are covered by coefficientwise polynomial certificates;
the finitely many boundary orders are solved exactly over $\mathbb Q$ by the
included replay.
...
ball around $J_n$ in the affine space $J_n+\calT_n$.  The smallest
$\Rtwo_{n}$-eigenvalue on the unit sphere is positive.  Taylor's theorem with
