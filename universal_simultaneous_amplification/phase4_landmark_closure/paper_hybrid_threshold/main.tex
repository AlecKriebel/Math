...
family suppresses at the endpoint \cite{Kriebel2026}.  The latter fact
suggested, but did not prove, that $3/2$ might be the universal phase
boundary.
...
zero at the admissible value $\sigma_*>0$.  Immediately above $\Rhyb$ the
discriminant is negative, so no positive $\sigma$---indeed no real
$\sigma$---is feasible.  Since the fixed pair $(\sigma_*,\lambda_*)$ works
throughout $(1,\Rhyb)$, no interval beginning at one can extend farther.
\end{proof}
...
$\theta\leq1/3$ \cite{Kriebel2026}.  Therefore no graph-independent convex
affine endpoint separator exists.  All signs here are exact rational signs
from two independent $61$-state solves; the displayed decimals are only
shorthand.
...
\textbf{EXACTLY REFUTED:}&$\Rsim=3/2$ and every graph-independent convex
affine endpoint separator of the displayed form.\\
...
manuscript is a public-repository research draft, but has no tagged preprint
release or archival DOI at this checkpoint.  A public preprint release would
not constitute peer-reviewed journal publication.
...
\bibitem{Kriebel2026}
A.~Kriebel,
\emph{A fitness-independent family of simultaneous amplifiers below fitness
three halves}, public preprint and exact certificate package (2026).
