...
calculations.  The archive generated by `release_bundle.sh` contains the
complete LaTeX source and PDF, exact verifiers, a paper-level integration
audit, pinned Python dependencies, deterministic build scripts, and an
internal SHA-256 manifest.  At the date of this draft, no new persistent identifier
has been assigned to the superseding manuscript package.  The final statement
must name its new versioned release and identifier only after those records
actually exist.
