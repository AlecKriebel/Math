...
Last updated: 2026-08-08 hostile-audited hybrid preprint release.
...
| weighted-leaf rescalings improve the dilute hybrid threshold | **FALSIFIED FOR THE AUDITED COMMON-HUB AND DISTINCT-HUB REGIMES** | exact killed-branching roots and far-field singleton Poisson term; `D+(r-1)B<0` for every `r>=3/2` in the distinct-hub heavy regime | more general integrated finite gadgets remain under active search |
...
| marked two-step `r=2` comparison | **PROVED; PROMOTION EXACTLY REDUCED** | one-sample stationary lift; uniform two-step rank-PGF gap is a nonnegative combination of row-collision, column-imbalance, and antisymmetry sums of squares; active Perron operator has size-weighted invariant mass and collision as unweighted Perron mass | prove the remaining stationary collision-observable promotion; generic late-time and rank-zero envelopes are false |
...
| exact value of `R_sim` | **OPEN; RIGOROUS LOWER BOUND `R_hyb>3/2`** | dilute hybrid theorem; no matching upper bound | improve the dilute tangent cone or prove a universal obstruction, with the `r=2` route still active |
