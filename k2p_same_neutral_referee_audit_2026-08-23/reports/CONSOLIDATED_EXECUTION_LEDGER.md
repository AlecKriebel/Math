...
This ledger records every substantive package, producer, verifier, build,
replay, and falsification command used for the scientific review. Read-only
...
| full-suite ledger | exact `argv` and results for all 22 full stages | `PENDING_FULL_RESULT` |
...
| `./setup_environment.sh` | isolated project | 0 | not retained | not retained | first required invocation succeeded; environment then moved aside for a clean repeat |
| `./setup_environment.sh` | isolated project | 0 | 10.33 s | 165,773,312 B | clean environment: Python 3.14.6, NetworkX 3.5, SymPy 1.14.0 |
| `/usr/bin/time -l .venv/bin/python -B run_all_verifiers.py --quick` | isolated handoff | 0 | 778.96 s | 1,460,994,048 B | 21/21; ledger `76236eebb4900c2aa3b616470d5a15fd9de9228c3fe0a5cdc43bd472bc9ef2cd` |
| `/usr/bin/time -l .venv/bin/python -B run_all_verifiers.py --full` | isolated handoff | `PENDING_FULL_RESULT` | `PENDING_FULL_RESULT` | `PENDING_FULL_RESULT` | invoked once; not restarted |
...
The stage-20 log contains all 20 nested theorem-layer commands with individual
return codes, runtimes and stdout/stderr hashes; its binding is given above and
the per-layer values are reproduced in
`reports/THEOREM_LAYER_EVIDENCE_REGISTRY.md`.
...
| # | Stage / exact relative `argv` | Exit | Wall s | Log SHA-256 |
|---:|---|---:|---:|---|
| 22 | `final_theorem_full_primitive_regeneration` — `.venv/bin/python -B work/final_theorem_release/verify_final_theorem_release.py --full --timeout-seconds 7200` | `PENDING_FULL_RESULT` | `PENDING_FULL_RESULT` | `PENDING_FULL_RESULT` |
...
| disposable weak-sharpness independent audit | 0 | 0.25 s | 43,302,912 B | `cfd8d3a2ebc7431d141cac6ebe943e25730eb086fbc84b52833a40bee40a5d52` |
...
| `python3 -B scripts/mathematical/verify_quartet_conventions.py` | 0 | about 0.39 s | not retained | `07af30a348f67a1449044ef5f2024f80e833b9cc682698ccd52969fe39bcc9bf` |
| `python3 -B scripts/mathematical/verify_triangle_germ.py` | 0 | about 0.36 s | not retained | `4801dabb3f602761da9450560e9baae62c9061973c3365fafb809a9d17008e88` |
| `python3 -B scripts/mathematical/verify_weak_sharpness.py` | 0 | about 0.57 s | not retained | `a18d67fd9858d217578df413714f3b9e9da88e0f39635f37003574806a3319d3` |

## Provenance/build/archive command ledger

The provenance JSON bound at the top records 29 command/event entries with
exact commands, working directories, exit statuses, retained runtimes, peak
RSS where available, and output hashes. This includes two clean archive
