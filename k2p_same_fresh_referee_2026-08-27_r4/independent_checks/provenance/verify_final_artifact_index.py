#!/usr/bin/env python3
"""Independently verify the retained R4 artifact index."""
...
def main() -> None:
    ...
    for path in sorted(ROOT.rglob("*")):
        ...
        if path.is_file():
            ...
            actual[relative.as_posix()] = {"bytes": len(data), "sha256": digest(data)}
    if actual != expected:
    ...
    print(json.dumps({"status": "PASS", "file_count": len(actual),
                      "content_root_sha256": index["content_root_sha256"],
                      "payload_sha256": payload}, sort_keys=True))
