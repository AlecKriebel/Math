#!/usr/bin/env python3
"""Build a deterministic index of retained review-owned R4 artifacts."""
...
def main() -> None:
    ...
    for path in sorted(ROOT.rglob("*")):
        ...
        if not relative.parts or relative.parts[0] in EXCLUDED_TOP:
            continue
        if path == OUTPUT or "__pycache__" in relative.parts or path.name in {".DS_Store"}:
            continue
        if path.is_symlink():
    ...
    result = {
        "schema": "k2p-r4-review-artifact-index-v1",
        "scope": "retained review-owned files; excludes isolated/execution/tmp/pdf_render and this index",
    ...
    print(json.dumps({key: result[key] for key in
                      ("file_count", "total_bytes", "content_root_sha256", "payload_sha256")},
                     sort_keys=True))
