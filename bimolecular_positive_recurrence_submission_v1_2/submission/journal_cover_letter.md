...
contains their positive-recurrence conclusion as a special case. The
comparison has been checked against their Section 6: Theorem 6.1 reduces the
argument to tier inclusion (11), Lemma 6.5 constructs the finite reaction
word, Lemma 6.3(ii) supplies the strict descent tending to minus infinity, and
Lemma 6.4 assembles the sampled-chain argument. Section 6.1, equations
(19)--(20), uses the extra assumption to force $S_v$ after D-tier maximality
excludes $2S_v$; the source propensity of $S_v$ supplies the required
comparison.
...
A self-contained deterministic verification package accompanies the paper.
...
and the complete statement at
`supplement/ai_use_full_statement.md` identifies the known tools, models,
...
0.3, 1.0, 1.1, and 1.2 are publicly accessible as unrefereed research
