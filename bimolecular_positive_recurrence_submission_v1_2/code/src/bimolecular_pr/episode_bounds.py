...
def scalar_envelope_pointwise_increment(
    q: Fraction,
    p: Fraction,
    lower_M: Fraction,
    upper_M: Fraction,
) -> Fraction:
    """Exact change in the scalar-envelope objective as ``M`` increases.

    The constant and logarithmic terms cancel, so the difference is the
    rational number ``q * p * (upper_M - lower_M)``.  Its nonnegativity for
    ``q > 0``, ``0 < p <= 1``, and ``lower_M <= upper_M`` is the pointwise
    proof that taking the supremum preserves monotonicity of ``F_q``.
    """
    if q <= 0:
        raise ValueError("q must be positive")
    if not 0 < p <= 1:
    ...
    if lower_M > upper_M:
        raise ValueError("lower_M must not exceed upper_M")
    return q * p * (upper_M - lower_M)
