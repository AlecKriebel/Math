...
def classify_top_complexes(
    ...
    if all(qK[y] == 1 for y in complexes):
        # In this branch y is top iff qK(y) == 1.  Thus this condition would
        # imply that every complex is top, which was returned above.
        raise AssertionError("all qK values are one only in the all-top case")
