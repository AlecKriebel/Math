...
reduces positive recurrence to their tier inclusion (11).  Lemma~6.5
constructs the finite reaction word, Lemma~6.3(ii) converts its strict D-tier
descent into a contribution tending to minus infinity, and Lemma~6.4 assembles
these ingredients into the sampled-chain argument.  The pure-species
assumption enters in Section~6.1, equations~(19)--(20), when a D-top mixed
complex \(S_u+S_v\) is disabled because the \(u\)-coordinate is zero while
the \(v\)-coordinate diverges.  The assumption forces \(S_v\) to be a complex,
after \(2S_v\) is excluded by D-tier maximality; \(S_v\) then supplies the
source-rate comparison needed for (11)
\citep[Section~6; Theorem~6.1; Lemmas~6.3--6.5; Section~6.1;
equations~(19)--(20)]{AndersonCappellettiKim2020}.  The marked-target
construction used here avoids that boundary-availability step: the carried
target is automatically enabled at the post-jump state, and a directed
target-following path propagates its reward until the top-complex alternative
supplies a rare terminal source.  It therefore does not require a unary or
pure-double complex for every species.
...
states exits the class.  In a nonabsorbing class, let \(J_1\) be the first
...
\begin{proof}
Suppose the channel \(y\to y'\) is enabled at \(x\).  Write \(x=r+y\) with
\(r\in\mathbb N_0^d\), so firing it gives \(x'=r+y'\).  Weak reversibility
supplies a directed complex path
\[
  y'=z_0\longrightarrow z_1\longrightarrow\cdots
      \longrightarrow z_m=y.
\]
At population \(r+z_j\), the source \(z_j\) is present, so the corresponding
channel is enabled and produces \(r+z_{j+1}\).  The path therefore lifts to
\[
  x'=r+z_0\longrightarrow r+z_1\longrightarrow\cdots
      \longrightarrow r+z_m=x.
\]
...
residual \(r\) is merely nonnegative.  It also applies channel by channel to
...
\begin{remark}[The boundary example of ACK]\label{rem:ack-example}
...
\]
On the example used by ACK to demonstrate failure of unshifted one-step
entropy drift, the carried-target episode therefore has strictly negative
logarithmic drift.  This is precisely what the target augmentation adds.
\end{remark}
...
\href{https://github.com/AlecKriebel/Math/tree/bimolecular-positive-recurrence-v1.2/bimolecular_positive_recurrence_submission_v1_2}{the tagged Version~1.2 repository directory}.
...
This is an unrefereed preprint prepared for bioRxiv posting and subsequent
journal submission.  Earlier versioned repository copies are publicly
available and are superseded by Version~1.2.
...
\href{https://github.com/AlecKriebel/Math/blob/bimolecular-positive-recurrence-v1.2/bimolecular_positive_recurrence_submission_v1_2/supplement/ai_use_full_statement.md}{supplement/ai\_use\_full\_statement.md}.
