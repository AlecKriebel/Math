...
## Version 1.2 submission candidate

This directory is the unrefereed Version 1.2 submission candidate frozen on
...
Version 1.2 is a separate, superseding release. Versions 0.3, 1.0, and 1.1 and
their Git history remain unchanged. The Version 1.1-to-1.2 relationship is
recorded in `preservation/VERSION_1.1_PROVENANCE.md`.

## Exact result and scope

Weak reversibility alone gives an elementary state-space fact. If an enabled
reaction takes $x=r+y$ to $x'=r+y'$, a directed complex path from $y'$ back to
$y$ lifts, with the same residual $r\geq0$, to enabled population transitions
...
- `manuscript/main_arxiv.pdf`: arXiv fallback PDF; do not use concurrently
  with bioRxiv.
- `manuscript/supplementary_note.pdf`: standalone technical supplement.
...
The verifier runs 57 mathematical and calibration tests, generates the
...
- `audit/`: the preserved Version 1.1 mathematical-audit entry point.
