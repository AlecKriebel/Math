...
def relation_payload(source, target, port_matching, direction="source_to_target", root_case="R1"):
    return {
        "fixed_full_root_case_id": root_case,
        "source_rooted_graph_id": source,
        "target_rooted_graph_id": target,
        "port_matching": port_matching,
        "direction": direction,
    }


def make_record(source, target, port_matching, polynomial, paths, children, root_case="R1"):
    payload = relation_payload(source, target, port_matching, root_case=root_case)
    return {
...
def validate(records, expected_ids, expected_polynomials):
    ...
    for r in records:
        payload = relation_payload(
            r["source_rooted_graph_id"], r["target_rooted_graph_id"],
            r["port_matching"], r["direction"], r["fixed_full_root_case_id"],
        )
        ...
        for p in r["paths"]:
            if p["fixed_full_root_case_id"] != r["fixed_full_root_case_id"]:
                raise ValueError("merge across fixed full root cases")
            if p["source_rooted_graph_id"] != r["source_rooted_graph_id"]:
                raise ValueError("merge across source rooted graph IDs")
            if p["target_rooted_graph_id"] != r["target_rooted_graph_id"]:
...
def main():
    ...
    records = [
        make_record("S1", "T1", [["L_0", "L_0"]], [[[0], 1]], [
            {"fixed_full_root_case_id": "R1", "source_rooted_graph_id": "S1", "target_rooted_graph_id": "T1", "path_id": "p1", "child_state_ids": ["c1", "c2"]},
            {"fixed_full_root_case_id": "R1", "source_rooted_graph_id": "S1", "target_rooted_graph_id": "T1", "path_id": "p2", "child_state_ids": ["c1", "c2"]},
        ], ["c1", "c2"]),
        make_record("S2", "T2", [["L_0", "L_0"]], [[[1], -1]], [
            {"fixed_full_root_case_id": "R1", "source_rooted_graph_id": "S2", "target_rooted_graph_id": "T2", "path_id": "q1", "child_state_ids": []},
        ], []),
    ]
    ...
    x = deepcopy(records); x[0]["source_rooted_graph_id"], x[0]["target_rooted_graph_id"] = x[0]["target_rooted_graph_id"], x[0]["source_rooted_graph_id"]; x[0]["direction"] = "target_to_source"; cases["reversed_direction"] = x
    ...
    x = deepcopy(records); x[0]["paths"][1]["fixed_full_root_case_id"] = "R2"; cases["cross_root_case_merge"] = x
    x = deepcopy(records); x[0]["paths"][1]["source_rooted_graph_id"] = "S9"; cases["cross_source_rooted_graph_merge"] = x
    x = deepcopy(records); x[0]["paths"][1]["target_rooted_graph_id"] = "T9"; cases["cross_target_rooted_graph_merge"] = x
