"""Self-contained exact machinery for the direct-anchor closure gate.

This is a mechanically vendored snapshot of the already clean-room
``final_probe_extension_adversary`` engine.  It deliberately imports no
project research implementation.  Sparse integer polynomials, rooted
validation, bridge detection, standard mixed reduction, displayed-tree
enumeration, JC Fourier pullbacks, graph maps, and modular Jacobian ranks are
implemented here from first principles.  The direct-anchor compiler and its
mutation audit live wholly in this directory.
"""
