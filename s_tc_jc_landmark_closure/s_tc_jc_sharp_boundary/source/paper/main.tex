...
same-type four-leaf level-2 comparisons and proves that generic identifiability
fails over the full weakly tree-child level-2 class, even modulo ordinary
...
The pair's location in the tree-child hierarchy is important for the scope of
the result.  Following the semi-directed
...
Englander et al.~\cite{EnglanderEtAl2026}.  It proves failure over the full
weakly tree-child class even modulo ordinary triangle redirection, but because
the pair also contains a triangle, it does not settle the triangle-free weakly
tree-child subclass.
...
studied by Currie et al.~\cite{CurrieEtAl2026}, and from the
stacked-reticulation and two-blob local modifications studied by
...
root to be the lowest stable ancestor of $X$.  A vertex is a stable ancestor
of $X$ if it lies on every directed path from the root to each labelled leaf;
thus no proper descendant of the root may have this property.  A rooted
network is
...
the same value $L=+\sqrt{BEH}$, and the five reconstruction formulas in the
proof of \cref{lem:geometry} force $J,K,M,N,O$.  Hence the intersection
contains a relative open
...
$X\cup\{a\}$ whose zero-total coordinates are strictly positive.  Replace
...
\]
because $P(0,0)=1$.  Choose any $g_X$ with total character $h$.  Strict
positivity of the zero-total JC Fourier coordinates permits division, and
\[
\frac uv=
\frac{\widetilde P(g_X,h,0)}{\widetilde P(g_X,0,h)}.
\]
Thus $u$ and $v$ are recovered uniquely by the positive square root.  Every
zero-total coordinate of $P$ is then recovered from \eqref{eq:cherry} by
division by a positive monomial in $u,v$.  These formulas are real analytic.  Therefore
...
omnian at $A$ is unchanged, so they remain outside $\TCs$.  Leaf $1$ remains
adjacent to a triangle vertex in $N_n$ and to a nontriangle vertex in $N'_n$.
This labelled property is preserved by mixed-graph isomorphism and by
$\Tmove$.  Thus the two enlarged
...
construction settles neither identifiability inside $\TCs$ nor identifiability
in the triangle-free subclass of $\TCw$; both remain outside the scope of this
paper.
