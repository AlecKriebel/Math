...
| Theorem 6.3, including every three-/four-outgoing relation and restoration/probe closure | `atlas/ATLAS_EVIDENCE_BINDINGS.jsonl.gz`; the exact streams named in each record | `verifiers/evidence_bindings.py`; `reviews/bounded_directed_relation_cleanroom/cleanroom_verify.py`; `reviews/final_hard_cover_cleanroom/audit_candidate_stream.py`; `reviews/compact_probe_clean_clone_gate/semantic_gate.py` |
