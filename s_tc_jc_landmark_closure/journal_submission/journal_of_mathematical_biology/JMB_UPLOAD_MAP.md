...
5. Upload `JMB_Cover_Letter.pdf` if the portal requests a cover letter.
6. Enter the metadata from `JMB_SUBMISSION_METADATA.md`.
7. Provide the author’s city and country if the current title-page/portal
   fields require them; this package intentionally does not invent them.
8. Insert the real public repository or persistent archive identifier.  Do
   not enter an unissued DOI.
9. Confirm the funding, competing-interest, data/code, contribution, and AI
   declarations.
10. Inspect the portal-generated proof, especially formulas, TikZ figures,
    references, supplement linkage, and author information.
11. Stop before final submission until `FINAL_HUMAN_CHECKLIST.md` is complete.
