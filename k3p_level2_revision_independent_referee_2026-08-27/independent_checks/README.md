...
| `check_restoration_probe_census.py` | Streams every restoration and probe row; recomputes file hashes, row/record hashes, ordered roots, counts, references, endpoint-map compatibility, registry usage, and strict witness margins | Stored ledgers, registries, and manifests | Does not reconstruct every restoration/probe graph; it is an all-row binding/census audit. |
