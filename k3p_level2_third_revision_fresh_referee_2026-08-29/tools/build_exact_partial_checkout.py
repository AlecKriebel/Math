#!/usr/bin/env python3
"""Build a self-contained exact sparse checkout for the sealed source commit."""
...
DESTINATION = AUDIT / "execution/release_engineering_738b/repo"
RUNTIME = DESTINATION.parent
GIT = "/opt/homebrew/bin/git"
COMMIT = "738b662aa9c4e6201277f60b249afd4de9bcd9d6"
...
def selected_object_ids() -> set[str]:
    ...
    for record in listing.split(b"\x00"):
        ...
        objects.add(object_id)
    require(len(objects) == 3578, ("unexpected selected object census", len(objects)))
    return objects


def main() -> int:
    require(not DESTINATION.exists(), ("destination already exists", DESTINATION))
    require(text(SOURCE, ["cat-file", "-t", COMMIT]).strip() == "commit",
            "source commit")
    objects = selected_object_ids()
    DESTINATION.mkdir(parents=True, exist_ok=False)
    git(DESTINATION, ["init", "-q"])
    descriptor, raw_pack = tempfile.mkstemp(
        prefix="selected-objects-", suffix=".pack", dir=RUNTIME
    )
    os.close(descriptor)
    ...
    try:
        ...
        with raw_pack_path.open("rb") as source:
            indexed = subprocess.run(
                ...
                env=environment(), check=False, timeout=900,
            )
        require(indexed.returncode == 0, ("index-pack", indexed.stderr[-2000:]))
        pack_hash = indexed.stdout.decode("ascii").strip().split()[-1]
    ...
    require(text(DESTINATION, ["rev-parse", "HEAD"]).strip() == COMMIT,
            "checkout HEAD")
    require(text(DESTINATION, [
        "status", "--porcelain=v1", "--untracked-files=all"
    ]) == "", "checkout status")
    require(git(DESTINATION, ["symbolic-ref", "-q", "HEAD"],
                check=False).returncode == 1, "checkout is not detached")
    paths = [path for path in DESTINATION.iterdir() if path.name != ".git"]
    require(paths == [DESTINATION / PREFIX], ("unexpected checkout roots", paths))
    tracked = text(DESTINATION, [
        "ls-tree", "-r", "--name-only", "HEAD", "--", PREFIX
    ])
    require(len(tracked.splitlines()) == 623, "tracked project census")
    ...
    require(not (DESTINATION / ".git/objects/info/alternates").exists(),
            "unexpected alternate")
    print(
        f"EXACT_PARTIAL_CHECKOUT_PASS objects={len(objects)} "
        f"pack={pack_hash} files=623"
    )
    return 0
