...
two complementary structural results.  Every finite-dimensional exact
maximizer of the first augmented family has, on the support of Alice's
reduced state, all \(d\) equality roots with equal multiplicity, so \(d\)
divides the supported dimension.  At the same time, conditional permutations
...
the scalar phases, finite-dimensional equal supported multiplicities, and a
conditional phase-permutation orbit with exact product hypotheses\\
...
\(2\csc(\pi/(2d))\).
\item Every finite-dimensional exact maximizer of the first augmented family
has all equality roots with equal multiplicity on Alice's support; in
particular, \(d\) divides that supported dimension.
...
The equality certificate contains more information about arbitrary exact
maximizers once the first cyclic operator is augmented by its aligned term.
The following result is restricted to an attained finite-dimensional tensor-
product maximum.  It does not extend silently to \(qa\) or \(qc\).
...
all finite-dimensional exact maximizers of the first augmented family; then
...
\]
It follows that \(U|_K=\omega^{-y}(S_y|_K)^2\), so \(K\) reduces \(U\).
...
Raising \eqref{eq:polar-stabilizer} to the \(d\)-th power and using
\(B_y^d=\Id\), then applying \eqref{eq:support-cancellation}, gives
\(V_y^d=\Id_K\).  Since \(\eta^d=-1\),
