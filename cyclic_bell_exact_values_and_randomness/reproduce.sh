...
export SOURCE_DATE_EPOCH=1786276800
...
echo "[1/11] Unified scalar, polar, strategy, permutation, SOS, and setting checks"
"$PYTHON_BIN" "$PACKAGE_DIR/verification/verify_merged.py"

echo "[2/11] Restored rigidity, source-benchmark, binary, and private-MUB checks"
"$PYTHON_BIN" "$PACKAGE_DIR/verification/verify_rigidity.py"
"$PYTHON_BIN" "$PACKAGE_DIR/verification/verify_exact_benchmarks.py"
"$PYTHON_BIN" "$PACKAGE_DIR/verification/verify_private_mub_binary.py"

echo "[3/11] Independent exact d=4 first-family certificate"
"$PYTHON_BIN" "$REPO_DIR/cyclic_randomness_counterexample/verify_exact.py"

echo "[4/11] Independent exact d=4 second-family certificate"
"$PYTHON_BIN" "$REPO_DIR/minimum_bell_randomness/verify_second_family_d4_exact.py"

echo "[5/11] Retained setting-complexity regressions"
...
echo "[6/11] Historical source-package integrity"
...
echo "[7/11] Canonical manuscript build"
...
echo "[8/11] Two-page reviewer-summary build"
...
echo "[9/11] PDF metadata and deployed asset checks"
...
echo "[10/11] Website metadata, redirects, sitemap, and local links"
"$PYTHON_BIN" "$PACKAGE_DIR/verification/verify_site.py"
xmllint --noout "$REPO_DIR/docs/sitemap.xml"

echo "[11/11] Canonical package integrity manifest"
