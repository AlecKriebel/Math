...
For every clause of the second-family permutation construction, continue with [GeneralSecondMoments.lean](CyclicBell/GeneralSecondMoments.lean), [GeneralSecondResiduals.lean](CyclicBell/GeneralSecondResiduals.lean), and [GeneralOutcomeRelabeling.lean](CyclicBell/GeneralOutcomeRelabeling.lean). These give the full `d × (d+1)` complex first-harmonic matrix directly from the Born behavior, every local first moment, and each literal SOS residual applied to the state. The relabeling module proves that outcome inversion gives Bob's adjoint and preserves the table up to `b→−b`; its value comparison explicitly uses the transported Bell functional. Observed maximum-entry and fixed-guess statements are kept distinct from worst-case Eve optimization. Expanded interfaces are in [GeneralSecondCompletionStatements.lean](CyclicBell/GeneralSecondCompletionStatements.lean).
...
[GeneralPermutation.lean](CyclicBell/GeneralPermutation.lean) states the conditional phase-permutation theorem. Its scalar cap, phase alignment, and cyclic-product hypotheses are part of the manuscript theorem. [GeneralCoveragePermutation.lean](CyclicBell/GeneralCoveragePermutation.lean) adds arbitrary-Hilbert bounds, local first moments, and every first-harmonic correlator for its generic **two-Alice-input** construction; [GeneralCoverageWitness.lean](CyclicBell/GeneralCoverageWitness.lean) exposes its simple-spectrum and complete first-harmonic packages. “All harmonics” in those theorem names means all first-harmonic entries, not every Fourier order.
