...
theorem linear_factor_relations (α β : R → ℂ) (M : ℝ)
  ...
  · intro r
    ...
    simpa only [map_star,map_mul,map_add,map_smul,map_one,matrixCfc_coordinate,
  ...
  · have h : star (linearGapCM U α β M)*linearGapCM U α β M=
      ...
      simp only [linearGapCM,ContinuousMap.coe_mk,Pi.star_apply,Complex.conj_ofReal,
...
def linearPermutationAlice {α β : R → ℂ} {M : ℝ}
  ...
  if x=0 then fourierMeasurement d else cycleMeasurement (p.z ∘ σ)
    (phases_permuted _ p.z_unit σ) (by rw [product_permuted]; exact p.z_product)
...
def linearPermutationBob {α β : R → ℂ} {M : ℝ}
  ...
  | some r => cycleMeasurement (fun j => star (p.s r (σ j)))
      ...
      (by rw [← map_prod,product_permuted,p.s_product]; simp)
...
theorem aligned_phase_conjugate {α β : R → ℂ} {M : ℝ}
  ...
  rw [← p.phase_alignment r j]
  calc
    p.s r j*(‖α r+β r*p.z j‖ : ℂ)*star (p.s r j)=
      (star (p.s r j)*p.s r j)*(‖α r+β r*p.z j‖ : ℂ) := by ring
...
theorem linear_permutation_attains {α β : R → ℂ} {M : ℝ}
  ...
  simp_rw [h0,h1,hb,hbr,hv,weighted_entry_conjugate,weighted_linear,phi_weighted,
