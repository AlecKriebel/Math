...
theorem binary_operator_sos (A₀ A₁ B₀ B₁ : Mat ι)
  ...
  simp only [Matrix.conjTranspose_add,Matrix.conjTranspose_sub,Matrix.conjTranspose_smul,
    ...
    add_mul,mul_add,sub_mul,mul_sub,smul_mul_assoc,mul_smul_comm,smul_smul,
    hA₀.2,hA₁.2,hB₀.2,hB₁.2,← hc00,← hc01,← hc10,← hc11]
  ext i j
  simp only [Matrix.add_apply,Matrix.sub_apply,Matrix.smul_apply,smul_eq_mul]
  push_cast
  field_simp
  linear_combination (3*(1 : Mat ι) i j)*(sqrt_three_square)
...
theorem effectKernelE_add (T : Matrix ι ε ℂ) (X Y : Mat ι) :
  ...
  simp [effectKernelE,Matrix.transpose_add,mul_add,add_mul]
...
theorem effectKernelE_left_stabilizer (T : Matrix ι ε ℂ) (S X : Mat ι)
  ...
  have hs : S.transpose*T.map star=T.map star := by
    ...
    simpa [Matrix.map_mul,hconj] using hh
  simp only [effectKernelE,Matrix.transpose_mul,mul_assoc,hs]
...
theorem binary_on_state_structure (T : Matrix ι ε ℂ) (A₀ A₁ B₀ B₁ : Mat ι)
  ...
  have hb : B₀*T=binaryXA A₀ A₁*T := by
    have he : B₀*T=((Real.sqrt 3 : ℂ)/3) • ((A₀+(2 : ℂ) • A₁)*T) := by
      ...
      simp only [binaryResidual₀,binaryResidual₁,add_mul,sub_mul,smul_mul_assoc,
        Matrix.add_apply,Matrix.sub_apply,Matrix.smul_apply,smul_eq_mul,Matrix.zero_apply] at h0 h1
      simp only [add_mul,smul_mul_assoc,Matrix.add_apply,Matrix.smul_apply,smul_eq_mul]
      linear_combination (-1/3 : ℂ)*h0+(-2/3 : ℂ)*h1
    simpa only [hratio,binaryXA,smul_mul_assoc] using he
  have hz : binaryZB B₀ B₁*T=A₀*T := by
    have hh := congrArg (fun R : Matrix ι ε ℂ => (1/(Real.sqrt 3 : ℂ)) • R) hr₀
    simpa [binaryZB,binaryResidual₀,add_mul,sub_mul,smul_mul_assoc,smul_add,smul_sub,smul_smul,hs]
      using hh
  have hXB : binaryXA A₀ A₁*B₀=B₀*binaryXA A₀ A₁ := by
    simp [binaryXA,add_mul,mul_add,smul_mul_assoc,mul_smul_comm,hc00,hc10]
  have hZA : binaryZB B₀ B₁*A₀=A₀*binaryZB B₀ B₁ := by
    simp [binaryZB,sub_mul,mul_sub,smul_mul_assoc,mul_smul_comm,hc00,hc01]
  have hXX : (binaryXA A₀ A₁*binaryXA A₀ A₁)*T=T := by
    rw [mul_assoc,← hb,← mul_assoc,hXB,mul_assoc,← hb,← mul_assoc,hB₀.2,one_mul]
  have hZZ : (binaryZB B₀ B₁*binaryZB B₀ B₁)*T=T := by
    rw [mul_assoc,hz,← mul_assoc,hZA,mul_assoc,hz,← mul_assoc,hA₀.2,one_mul]
  have hxpoly : A₀*binaryXA A₀ A₁+binaryXA A₀ A₁*A₀=
      (Real.sqrt 3 : ℂ) • (binaryXA A₀ A₁*binaryXA A₀ A₁-1) := by
    unfold binaryXA
    simp only [mul_add,add_mul,smul_mul_assoc,mul_smul_comm,smul_add,smul_sub,
      smul_smul,hA₀.2,hA₁.2]
    ext i j
    simp only [Matrix.add_apply,Matrix.sub_apply,Matrix.smul_apply,smul_eq_mul]
    field_simp
    linear_combination ((A₀*A₁+A₁*A₀) i j + (Real.sqrt 3 : ℂ)*(1 : Mat ι) i j)*sqrt_three_square
  have hzpoly : binaryZB B₀ B₁*B₀+B₀*binaryZB B₀ B₁=
      (Real.sqrt 3 : ℂ) • (binaryZB B₀ B₁*binaryZB B₀ B₁-1) := by
    unfold binaryZB
    simp only [mul_sub,sub_mul,smul_mul_assoc,mul_smul_comm,smul_sub,
      smul_smul,hB₀.2,hB₁.2]
    ext i j
    simp only [Matrix.add_apply,Matrix.sub_apply,Matrix.smul_apply,smul_eq_mul]
    field_simp
    linear_combination ((B₀*B₁+B₁*B₀) i j - (Real.sqrt 3 : ℂ)*(1 : Mat ι) i j)*sqrt_three_square
  refine ⟨hb,hz,hXX,hZZ,?_,?_⟩
  · rw [hxpoly,smul_mul_assoc,sub_mul,hXX,one_mul,sub_self,smul_zero]
  · rw [hzpoly,smul_mul_assoc,sub_mul,hZZ,one_mul,sub_self,smul_zero]
...
theorem binary_private_moments (T : Matrix ι ε ℂ) (A₀ A₁ B₀ B₁ : Mat ι)
  ...
  have hanti : (binaryXA A₀ A₁*A₀)*T=((-1 : ℂ) • (A₀*binaryXA A₀ A₁))*T := by
    have hh := hantiX
    rw [add_mul] at hh
    simpa [neg_smul,smul_mul_assoc] using eq_neg_of_add_eq_zero_right hh
...
theorem binaryEffect_idempotent (A : Mat ι) (hA : A*A=1) (a : Fin 2) :
  ...
  fin_cases a <;> simp [binaryEffect,smul_mul_assoc,mul_smul_comm,smul_smul,add_mul,mul_add,hA]
  all_goals module
...
theorem binaryEffect_orthogonal (A : Mat ι) (hA : A*A=1) (a b : Fin 2) (hab : a≠b) :
  ...
  all_goals simp [binaryEffect,smul_mul_assoc,mul_smul_comm,smul_smul,add_mul,mul_add,hA]
  all_goals module
...
theorem binary_saturation_privacy (T : Matrix ι ε ℂ) (hT : frobeniusSq T=1)
  ...
  have hcomm : binaryEffect A₀ a*binaryEffect B₀ b=binaryEffect B₀ b*binaryEffect A₀ a := by
    unfold binaryEffect
    simp only [smul_mul_assoc,mul_smul_comm,smul_smul,add_mul,mul_add,one_mul,mul_one,hc.1]
    module
  ...
  have hId : (binaryEffect A₀ a*binaryEffect B₀ b)*(binaryEffect A₀ a*binaryEffect B₀ b)=
    ...
    calc _=(binaryEffect A₀ a*binaryEffect A₀ a)*(binaryEffect B₀ b*binaryEffect B₀ b) := by
           rw [mul_assoc,← hcomm,mul_assoc]; noncomm_ring
         _=_ := by rw [binaryEffect_idempotent A₀ hA₀.2,binaryEffect_idempotent B₀ hB₀.2]
  rw [conditionalE_effectKernel T _ hHerm hId]
  unfold binaryEffect
  simp only [smul_mul_assoc,mul_smul_comm,smul_smul,add_mul,mul_add,one_mul,mul_one,
