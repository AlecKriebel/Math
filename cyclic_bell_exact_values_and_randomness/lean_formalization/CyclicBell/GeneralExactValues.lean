...
/-- The d=4 value requested in the original pilot, now over each actual model. -/
theorem first_four_augmented_radical_values :
    betaQ (firstAugmentedBell (d := 4))=2*Real.sqrt (4+2*Real.sqrt 2)+1 ∧
    betaQa (firstAugmentedBell (d := 4))=2*Real.sqrt (4+2*Real.sqrt 2)+1 ∧
    betaQc (firstAugmentedBell (d := 4))=2*Real.sqrt (4+2*Real.sqrt 2)+1 := by
  have h := first_augmented_values_q_qa_qc (d := 4) (by norm_num)
  change betaQ (firstAugmentedBell (d := 4))=scalarMaximum 4+1 ∧
    betaQa (firstAugmentedBell (d := 4))=scalarMaximum 4+1 ∧
    betaQc (firstAugmentedBell (d := 4))=scalarMaximum 4+1 at h
  simpa only [scalarMaximum_four] using h
