# Manuscript coverage — adversarial optimization and source Fourier continuation
...
4. Standard Fourier-phase self-test probability tables, anchored cross-table
   formulas and their asymptotic entropy comparisons in `app:settings`.
...
The scanner finds **1,090 theorem candidates**, 357 definitions,
14 abbreviations, 16 structures and 2 named instances.
**1,479 pending axiom queries** cover the explicitly named source declarations.
There are **92 expanded statement examples** and **76 Lean files** in the normal
build graph. The new extension adds 103 theorem candidates and ten Lean files.
These are source counts, not accepted-theorem or quality measurements.

The only earlier mathematical files changed are `GeneralFourier.lean` and
`GeneralWitness.lean`, for the unchanged-lemma relocation above. Existing
`GeneralStatements.lean` and `AxiomAudit.lean` changed for integration. All
other earlier mathematical source and dependency lockfiles are unchanged.

Executed tests are recorded in `CONTINUATION_REPORT.json` and `logs/`.
No Lean process, clean Lean build, actual axiom report or independent-agent
statement review occurred. Finite exact tests cannot verify continuity,
compactness, closure, suprema or universal proof scripts. Offline repairs
may be substantive. Historical coverage is preserved under `history/`.
