...
The recorded run was performed in a separate, standalone copy of this package using the bundled manuscript and `python3 scripts/check.py --bootstrap`. The pinned dependency checkouts and cache were supplied locally, then checked at their locked revisions; the bootstrap cache command was exercised. The companion itself was built from a clean build directory. No parent repository or manuscript path was needed.
...
[Statement review](STATEMENT_REVIEW.md) describes the separate manuscript correspondence assessment. [Source preservation](source_preservation.json) records the comment-only cleanup against the previously verified proof sources. Neither document substitutes for running Lean.

## Exporting a verified copy

```sh
python3 scripts/package.py --output ../cyclic-bell-lean-review.zip
```
