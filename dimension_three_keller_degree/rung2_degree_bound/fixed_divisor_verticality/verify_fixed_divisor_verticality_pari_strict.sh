...
output_file=$(mktemp "${TMPDIR:-/tmp}/fixed-divisor-verticality-pari.XXXXXX")
trap 'rm -f "$output_file"' EXIT HUP INT TERM

if ! gp -q -s 134217728 \
    "$script_dir/verify_fixed_divisor_verticality_pari.gp" \
...
expected='PASS: independent PARI/GP fixed-divisor verticality reconstruction'
