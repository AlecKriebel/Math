...
wrapper="$theorem_directory/verify_nonbinary_fixed_linear_line_pari_strict.sh"
fake_path="$audit_directory/fakebin:$PATH"

run_mode() {
    mode=$1
    expected=$2
    PATH="$fake_path" AUDIT_FIXED_LINEAR_FAKE_MODE="$mode" \
...
printf '%s\n' "AUDIT_SUPPLIED_FIXED_LINEAR_WRAPPER_SELFTEST_PASS"
