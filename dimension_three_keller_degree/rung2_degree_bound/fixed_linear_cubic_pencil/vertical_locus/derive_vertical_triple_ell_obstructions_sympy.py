#!/usr/bin/env python3
"""Derive simple E6 compatibility obstructions on triple-root ell branches."""
...
for label, (q, combination) in charts.items():
    H2 = sp.Matrix((A, B, W))
    ...
    y_obstruction = sp.factor(determinant.coeff_monomial(x**5 * z))
    x_obstruction = sp.factor(
        sum(
            coefficient * determinant.coeff_monomial(monomial)
            for coefficient, monomial in combination
        ).subs(v, 0)
    )
    print(label, "v obstruction", y_obstruction)
    print(label, "u obstruction", x_obstruction)
