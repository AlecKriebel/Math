...
output_file=$(mktemp "${TMPDIR:-/tmp}/audit-nonzero-ell.XXXXXX")
optimized_output=$(mktemp "${TMPDIR:-/tmp}/audit-nonzero-ell-opt.XXXXXX")
trap 'rm -f "$output_file" "$optimized_output"' EXIT HUP INT TERM

for source_file in \
    "$script_dir/verify_vertical_nonzero_ell_nontriple_sparse.py" \
...
expected='PASS: independent sparse audit of nonzero-ell nontriple lemma'
