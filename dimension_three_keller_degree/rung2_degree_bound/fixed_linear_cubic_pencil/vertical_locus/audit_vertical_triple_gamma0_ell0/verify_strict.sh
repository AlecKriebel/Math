...
output_file=$(mktemp "${TMPDIR:-/tmp}/audit-triple-zero.XXXXXX")
optimized_output=$(mktemp "${TMPDIR:-/tmp}/audit-triple-zero-opt.XXXXXX")
trap 'rm -f "$output_file" "$optimized_output"' EXIT HUP INT TERM

for source_file in \
    "$script_dir/verify_vertical_triple_gamma0_ell0_sparse.py" \
...
expected='PASS: independent sparse audit of all triple gamma=ell=0 charts'
