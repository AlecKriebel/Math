#!/usr/bin/env python3
"""Derive E6 data for h=(p-wq)(wp-q), R=h(Ap+Bq)."""
...
for rows in itertools.combinations(range(6), 4):
    for cols in itertools.combinations(range(5), 4):
        ...
        if value:
            print("contact rank4 decisive", rows, cols, value)
            found_rank_minor = True
            kernel_rows = rows
            break
    if found_rank_minor:
        break
submatrix = contact.extract(kernel_rows, range(5))
cofactors = []
for column in range(5):
    remaining = [index for index in range(5) if index != column]
    cofactors.append(
        sp.factor((-1) ** column * submatrix[:, remaining].det())
    )
cofactor_gcd = sp.factor(sp.gcd_list(cofactors))
primitive_kernel = [
    sp.factor(value / cofactor_gcd) for value in cofactors
]
print("contact cofactor gcd", cofactor_gcd)
print("contact primitive kernel", primitive_kernel)
print(
    "contact Veronese obstruction",
    sp.factor(
        primitive_kernel[1] ** 2
        - primitive_kernel[0] * primitive_kernel[2]
    ),
)
print(
    "contact primitive residual",
    [sp.factor(value) for value in contact * sp.Matrix(primitive_kernel)],
)
maximal = []
