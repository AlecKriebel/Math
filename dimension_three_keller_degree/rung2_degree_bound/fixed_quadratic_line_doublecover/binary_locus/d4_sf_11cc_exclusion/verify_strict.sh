...
"$keller_python" verify_exclusion_sympy.py | tee "$sympy_output"
grep -Fx "D4_SF_11CC_SYMPY_STRICT_PASS" "$sympy_output" >/dev/null
...
"$keller_gp" -q -s 256M verify_exclusion_pari.gp 2>&1 | tee "$pari_output"
grep -Fx "D4_SF_11CC_PARI_STRICT_PASS" "$pari_output" >/dev/null
if grep -E '^[[:space:]]*\*\*\*' "$pari_output" >/dev/null; then
  echo "FAIL: PARI/GP reported an error" >&2
  exit 1
fi

sed 's/-64-112\*root_symbol\/3/-63-112*root_symbol\/3/' \
  verify_exclusion_pari.gp >"$pari_fault_script"
"$keller_gp" -q -s 256M "$pari_fault_script" >"$pari_fault_output" 2>&1 || true
if grep -Fx "D4_SF_11CC_PARI_STRICT_PASS" "$pari_fault_output" >/dev/null; then
  echo "FAIL: mutated PARI/GP certificate was accepted" >&2
  exit 1
fi
grep -E '^[[:space:]]*\*\*\*' "$pari_fault_output" >/dev/null

echo "D4_SF_11CC_FULL_STRICT_PASS"
