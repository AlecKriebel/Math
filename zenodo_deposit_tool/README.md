...
Metadata fields must match exactly, with two narrow representation exceptions in the description. For plain text without HTML markup or existing character references, Zenodo may encode `&`, `<`, and `>` as HTML entities. For a description consisting solely of plain `<p>...</p>` paragraphs, Zenodo may change every U+2019 curly apostrophe to its ASCII counterpart. The verifier accepts only those exact transformations and reports them in `metadata_normalizations`. It does not ignore substantive text changes, strip tags, alter attributes, double-encode existing entities, normalize other punctuation, or normalize other fields. The local manifest and supplied metadata remain unchanged.
