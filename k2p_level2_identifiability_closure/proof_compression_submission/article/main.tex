...
\cite{BritsEtAl2026}.  Three-sunlet group-based varieties have also been
...
At level two the first broad structural result is due to Englander et
al.~\cite{EnglanderEtAl2026}.  Their topology-only primitive and repair
theorems reduce strongly tree-child blobs to a finite family of cycle and
directed theta cores.  Their Propositions~2.9--2.10 and Theorem~2.11 apply to
JC or K2P and show that different displayed-quartet sets are pointwise
separated; Corollary~2.12 recovers the labelled tree of blobs.  Their main
level-2 algebraic theorem is for JC.  The present work retains their graph
layer but rebuilds every algebraic layer for K2P, whose nonzero Fourier
characters split into two orbits rather than one.
...
\begin{align}
 F_A&=q_{GGGG}-q_{GGTT},\label{eq:quartet-F}\\
...
\begin{proof}
Substitution in the tree monomial formula \eqref{eq:k2p-fourier} cancels the
two terms of \(F_A\) on split \(A\).  On either crossing split, their
difference is a positive pendant monomial times \(1-a\), where \(a\in(0,1)\)
is the effective paired-sector internal multiplier.  This proves the first
claim.  The same direct substitution shows that \(G_B\) cancels on \(A,C\)
and is a positive pendant monomial times \(1-a\) on \(B\).  Leaf
permutations give \(F_t,G_t\) for every split \(t\).
...
\subsection{The whole-map \(\mathcal T_i\) polynomial}
...
\Cref{lem:marginal}, together with the identity on other source parameters,
gives a physical source-open image.  The actual attachments and transport of
the fixed networks are one row of the exhaustive child construction.
...
between the two degree-three poles, hence a theta.  Acyclic binary placement
of the unique incoming source and the two reticulation events gives four
possibilities: one reticulate pole with source and internal sink on different
or equal paths (\(\theta_0,\theta_1\)); or two internal sinks with the source
on the third path or sharing a sink path (\(\theta_2,\theta_3\)).  This is the
topology-only primitive theorem of Englander et
al.~\cite{EnglanderEtAl2026}.  Reading port vertices along the oriented
segments recovers the unique words.
\end{proof}
...
\end{tabular}
\end{center}
Occupancy is monotone; each listed minimal repair gives an ordinary child to
every offending tail, separates a reticulation child, or splits a parallel
core pair.  The reader supplement records the exact completion grammar.

For a target core \(H\), let \(m_H\) be its number of directed segments,
\(q_H\) its path-sink count, and \(r_H\) its number of minimum repair
choices.  If \(k\) physical boundaries are selected and
...
\end{equation}
Indeed, choose the \(j\) selected path sinks and weakly distribute the
remaining \(k-\epsilon-j\) selected boundaries among the \(m_H\) ordered
segments.  The five tuples \((m_H,q_H,r_H)\) are
\[
...
among directed segments, every minimum strong repair, and either a selected
incoming boundary or a marginalized incoming role.  It produces 831
...
\begin{center}
\small
\begin{tabular}{p{.25\linewidth}p{.27\linewidth}p{.38\linewidth}}
\toprule
...
five anisotropic output directions.  In logarithmic edge coordinates, exact
minors of the two blocks have determinants \(-1/2\) and \(-1/4\); their
product is \(1/8\).  Thus every orientation has rank nine.  The inverse
function theorem gives strict analytic sections over one common output
neighborhood.  Notice from \eqref{eq:Ti-factor} that this common germ remains
strictly separated from the three-leaf tree model.
\end{proof}
...
sections of every orientation.  Let \(C\) be the parameter space of the
identical exterior context and write its contraction as
\[
...
\]
Thus \(\mathcal H(U\times C)\) is the same full-dimensional contextual germ,
even if the three arms reconnect inside another level-2 factor.  For several
triangles use the product of all \(Q_j,U_j\) and make the rank choice once;
no induction assumes that an already chosen context remains generic.
...
\cite[Theorem~2.2.1]{BochnakCosteRoy1998}.  Choose finite Nash
...
semialgebraic dimension at most \(d_N-1\), and its Zariski closure inside
\(\V_N\) is proper.
...
then proper.  For the source Jacobian-rank-drop strata, instead adjoin the
Zariski closures of their images; every such restricted map has rank strictly
less than \(d_N\), so its image has smaller dimension.  Every added
set is therefore a proper closed subset of \(\V_N\); a finite union of such
...
\begin{enumerate}[label=\textbf{R\arabic*.}]
  ...
  \item Evaluate the bounded local certificate deck and select one rigid
        physical support.
  \item Follow only exact fixed-full restoration records and their stored
        transports; the depth is at most two.
  \item Use the certified one-port segment deck and two-port order deck to
        reconstruct every labelled subdivision word.
  \item Assemble the coherent labelled mixed graph, quotient its ordinary
        triangle choices, and return a canonical representative.
\end{enumerate}
...
Order each normalized map's parameter columns as
\((s_0,g_0,s_1,g_1,\ldots,s_6,g_6,\lambda_0,\lambda_1)\), with edge classes
in the canonical descendant-signature order stored by the exact certificate.
For \(W\), rows \((1,2,3,5,4,7,6,8,9)\) of \eqref{eq:sharp-order} and columns
\((0,1,\ldots,8)\) give determinant
\[
 \frac{10368019213741323}
 {563981315074464023964442388464888915634290688}\ne0.
\]
For \(W'\), rows \((1,2,3,5,4,6,7,8,9)\) and the same columns give
\[
...
The exact certificate and replay files are contained in the accompanying
repository under \path{k2p_level2_identifiability_closure}.  A final public
repository URL, immutable tag or DOI, archive SHA-256, and paper/data/code
license statements are \textbf{pending human confirmation}.  This draft does
not infer or assert a license.  The reader supplement provides a relative-path
crosswalk that remains valid in the local reproducibility tree.
...
submission.  This draft contribution statement is pending explicit human
confirmation.

\section*{Funding}

Funding statement: \textbf{pending human confirmation}.  This draft makes no
funding assertion.

\section*{Competing interests}

Competing-interests declaration: \textbf{pending human confirmation}.  This
draft makes no competing-interests assertion.
...
\section*{Acknowledgment of pending submission metadata}

The corresponding email address, author-contribution statement, funding
statement, competing-interests declaration, final public repository/DOI, and
all license choices remain pending explicit human confirmation and must be
