...
on the third path or sharing a sink path (\(\theta_2,\theta_3\)).  Deleting
the incoming arm and suppressing its resulting degree-two source yields one
of the two semi-directed level-2 generators classified by Huber et
al.~\cite[Lemma~4.2 and Figure~8]{HuberEtAl2025}.  The four-way
incoming-source refinement is established by the preceding case analysis.
...
\path{k2p_level2_identifiability_closure}.  An immutable submission tag or
DOI and paper/data/code license statements are \textbf{pending human
confirmation}.  This draft does not infer or assert a license.  The reader
...
statement, competing-interests declaration, immutable release tag/DOI, and all
license choices remain pending explicit human confirmation and must be
