...
def reset (j : Fin 5) : Endomorphism where
  ...
  map_add' := by
    ...
    fin_cases j <;> funext i <;> fin_cases i <;>
      norm_num [Pi.add_apply] <;> ring
  map_smul' := by
    intro t x
    fin_cases j <;> funext i <;> fin_cases i <;>
      norm_num [Pi.smul_apply, smul_eq_mul] <;> ring
...
theorem compatibility_id (B : Bilinear) (λ : Fin 5 → ℝ)
    (hnull : ∀ k, B (ray k) (ray k) = 0) :
    compatibility B λ LinearMap.id = 0 := by
  simp [compatibility, hnull]

/-- Exact gap identity with the sign of the paper's stationarity equation. -/
theorem stationary_gap (B : Bilinear) (λ : Fin 5 → ℝ)
    (hnull : ∀ k, B (ray k) (ray k) = 0)
    (F : Endomorphism →ₗ[ℝ] ℝ) (l : V →ₗ[ℝ] ℝ) (α : ℝ)
    (stationary : ∀ W, F W = α * l (W unitVector) - 2 * compatibility B λ W)
    (j : Fin 5) :
    F LinearMap.id - F (reset j) = 2 * λ j * B (ray j) unitVector := by
  rw [stationary LinearMap.id, stationary (reset j),
    compatibility_id B λ hnull, compatibility_reset B λ hnull,
...
theorem multipliers_positive (B : Bilinear) (λ : Fin 5 → ℝ)
    (hnull : ∀ k, B (ray k) (ray k) = 0)
    (hfuture : ∀ j, 0 < B (ray j) unitVector)
    (F : Endomorphism →ₗ[ℝ] ℝ) (l : V →ₗ[ℝ] ℝ) (α : ℝ)
    (stationary : ∀ W, F W = α * l (W unitVector) - 2 * compatibility B λ W)
    (strictGap : ∀ j, F (reset j) < F LinearMap.id) : ∀ j, 0 < λ j := by
  intro j
  have hpos : 0 < 2 * λ j * B (ray j) unitVector := by
    rw [← stationary_gap B λ hnull F l α stationary j]
    exact sub_pos.mpr (strictGap j)
  by_contra! hnonpos
  have hleft : 2 * λ j ≤ 0 := by linarith
...
theorem multiplier_formula (B : Bilinear) (λ : Fin 5 → ℝ)
    (hnull : ∀ k, B (ray k) (ray k) = 0)
    (F : Endomorphism →ₗ[ℝ] ℝ) (l : V →ₗ[ℝ] ℝ) (α : ℝ)
    (stationary : ∀ W, F W = α * l (W unitVector) - 2 * compatibility B λ W)
    (j : Fin 5) (hj : B (ray j) unitVector ≠ 0) :
    λ j = (F LinearMap.id - F (reset j)) / (2 * B (ray j) unitVector) := by
  apply (eq_div_iff (mul_ne_zero (by norm_num : (2 : ℝ) ≠ 0) hj)).2
  calc
    λ j * (2 * B (ray j) unitVector) = 2 * λ j * B (ray j) unitVector := by ring
    _ = F LinearMap.id - F (reset j) :=
      (stationary_gap B λ hnull F l α stationary j).symm
