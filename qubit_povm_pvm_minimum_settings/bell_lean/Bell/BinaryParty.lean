...
private theorem effectSpan_rank_le_support {n : ℕ} (N : POVM n) :
  ...
  change Module.finrank ℝ (effectSpan N) + Module.finrank ℝ (LinearMap.ker (activeEffectMap N)) = Module.finrank ℝ (EffectSupport N → ℝ) at h
...
theorem binary_span_sum_le_three (BO : Fin 2 → ℕ) (N : (y : Fin 2) → PVM (BO y))
    ...
    Module.finrank ℝ (effectSpan (N 0).toPOVM ⊔ effectSpan (N 1).toPOVM : Submodule ℝ V) ≤ 3 := by
  let S : Submodule ℝ V := effectSpan (N 0).toPOVM
  let T : Submodule ℝ V := effectSpan (N 1).toPOVM
  ...
  have hi : 0 < Module.finrank ℝ (S ⊓ T : Submodule ℝ V) := by
    by_contra! h
    have hz : Module.finrank ℝ (S ⊓ T : Submodule ℝ V)=0 := by omega
  ...
  change Module.finrank ℝ (S ⊔ T : Submodule ℝ V) ≤ 3
  omega

abbrev PieceIndex (AO : Fin 2 → ℕ) := (x : Fin 2) × (Fin (AO x) × Fin 2)
...
private theorem regroup_live_pieces (AO BO : Fin 2 → ℕ)
  ...
  classical
  let F : PieceIndex AO → V := fun j => if h : j.1=x then
    (if Fin.cast (congrArg AO h) j.2.1=b then pieceVector AO BO a n d j else 0) else 0
  have hlive : ConeCircuits.outcomeVector AO
      (fun j : {j : PieceIndex AO // pieceVector AO BO a n d j ≠ 0} => pieceVector AO BO a n d j)
      (fun j => j.val.1) (fun j => j.val.2.1) fallback (fun _ => 1) x b =
      ∑ j : {j : PieceIndex AO // pieceVector AO BO a n d j ≠ 0}, F j := by
    apply Finset.sum_congr rfl
    intro j _
    by_cases hi : j.val.1=x <;> simp [F, hi, ConeCircuits.declaredLabel]
  have hzero : ∀ j : {j : PieceIndex AO // ¬ pieceVector AO BO a n d j ≠ 0}, F j = 0 := by
    intro j
    have hz := not_not.mp j.property
    simp [F, hz]
  have hsum := Fintype.sum_subtype_add_sum_subtype
    (fun j : PieceIndex AO => pieceVector AO BO a n d j ≠ 0) F
  have he : (∑ j : {j : PieceIndex AO // pieceVector AO BO a n d j ≠ 0}, F j) =
      ∑ j : PieceIndex AO, F j := by
    simpa only [hzero, Finset.sum_const_zero, add_zero] using hsum
  rw [hlive,he]
  simp [F, Fintype.sum_sigma, Fintype.sum_prod_type, pieceVector, nullPiece_sum]
  simpa [Finset.sum_add_distrib, Fin.sum_univ_two] using
    nullPiece_sum d (compress n (coordinates (a.steered x b)))
...
theorem binary_bob_simulation (AO BO : Fin 2 → ℕ) (s : Strategy ⟨2,2,AO,BO⟩)
  ...
  classical
  let a : Assemblage ⟨2,2,BO,AO⟩ := (swapStrategy s).toAssemblage
  let S : Submodule ℝ V := effectSpan (N 0).toPOVM ⊔ effectSpan (N 1).toPOVM
  obtain ⟨n,hn0,hnunit,hnkill⟩ := exists_unit_annihilator S
    (binary_span_sum_le_three BO N hc) (Submodule.mem_sup_left (timeUnit_mem_effectSpan (N 0).toPOVM))
  obtain ⟨d,hd0,hdunit,hnd⟩ := exists_spatial_unit_perpendicular n
  have hnN : ∀ y b, dotProduct n (coordinates ((N y).effect b))=0 := by
    intro y b
    apply hnkill
    fin_cases y
    · exact Submodule.mem_sup_left (effect_coordinates_mem_span _ b)
    · exact Submodule.mem_sup_right (effect_coordinates_mem_span _ b)
...
theorem extreme_binary_bob_simulation (AO BO : Fin 2 → ℕ)
    ...
    s.behavior ∈ convexPVM ⟨2,2,AO,BO⟩ := by
  let ss : FullPureStrategy ⟨2,2,BO,AO⟩ := s.swap
  have hexs : ss.behavior ∈ Set.extremePoints ℝ (convexPOVM ⟨2,2,BO,AO⟩) := by
    change s.swap.behavior ∈ _
    rw [s.swap_behavior]
    exact swap_extreme hex
  let N : (y : Fin 2) → PVM (BO y) := fun y =>
    (s.bob y).toPVMOfTwoNull
      (extreme_nondeterministic_effects_null ss hexs y 0 (hnd y))
