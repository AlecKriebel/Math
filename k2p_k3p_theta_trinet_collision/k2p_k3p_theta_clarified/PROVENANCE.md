...
Version 1.2.6 adds the closest earlier level-two algebraic study to the
literature map. Ardiyansyah's 2021 Fourier-invariant work concerns restricted
simple and semisimple nice strict level-two JC/K2P/K3P varieties; its own
Lemma 5.1 excludes two- and three-leaf simple strict level-two networks from
that nice class, so it neither anticipates nor contradicts the present
pointwise non-nice theta-trinet collision. The broad Version 2-to-3 wording in
the abstract and acknowledgment now names the removed formal K2P lemma and the
K2P part of its corresponding global corollary. All verifier certificate
loads now reject duplicate object keys, nonstandard numeric constants, and
departures from hard-coded closed structural schemas, including exact
