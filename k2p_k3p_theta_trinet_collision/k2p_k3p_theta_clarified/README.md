...
The current submission snapshot is version `1.2.7`, published under the Git tag
`k2p-k3p-theta-v1.2.7`. The tag, rather than the mutable `main` branch,
...
submission or release inputs. Frozen versions `1.2.6`, `1.2.5`, `1.2.4`, `1.2.3`, `1.2.2`,
`1.2.1`, `1.2.0`, and `1.1.0` are retained under the parent `releases/` directory, and version
...
all 64 patterns, binds the complete canonical vertex and ten-arc schemas,
resolves every reticulation descriptor against its actual arc, binds every
advertised Jacobian and tangent label to its executable descriptor, and is
guarded by coordinated semantic-mutation tests. The compact K2P replay checks
every stored transition row, including the composed root-suppressed row.
All five certificate inputs use one strict loader that rejects duplicate JSON
keys and nonstandard numeric constants, and hard-coded structural fingerprints
reject added, removed, or structurally altered fields at every nesting level.
The hostile JSON suite checks these gates in normal and optimized modes.
