...
\end{tabularx}
\end{center}
If an edge has no labelled descendant, its Fourier label is $A$, so its factor is $U_A=1$ or $V_A=1$. Equivalently, that dangling branch disappears when the displayed tree is pruned to the labelled leaves. Each row has inheritance weight $1/4$, so after extracting the four common $K$-edge factors,
\[
...
\]
Writing $s=\sqrt{71}$ and
\[
P=(1,\tfrac{151}{36s},\tfrac{107}{162},\tfrac{151}{36s}),\qquad
R=(1,\tfrac{s}{40},\tfrac{31}{120},\tfrac{s}{40}),
\]
exact arithmetic gives $M_{y,z}=P_{y+z}R_yR_z$ for all $y,z$. The comparison-tree vectors are
\[
\alpha=K^{\odotv2}\odotv P=(1,\tfrac{151s}{10224},\tfrac{107}{648},\tfrac{151s}{10224}),\qquad
\beta=\gamma=K\odotv R=(1,\tfrac{s}{80},\tfrac{31}{240},\tfrac{s}{80}).
\]
Hence for every consistent $(x,y,z)$,
\[
q^{N_\theta}_{xyz}=K_x^2K_yK_zM_{y,z}=\alpha_x\beta_y\gamma_z=q^T_{xyz}.
\]
