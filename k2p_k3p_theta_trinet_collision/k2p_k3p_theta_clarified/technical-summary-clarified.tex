...
\textbf{Status and replay.} The canonical manuscript, exact certificates, and verifier package are available in the \href{https://github.com/AlecKriebel/Math/tree/k2p-k3p-theta-v1.2.3/k2p_k3p_theta_trinet_collision/k2p_k3p_theta_clarified}{versioned GitHub snapshot (v1.2.3)}. With Python 3.10 or newer, run \texttt{python3 verify\_k2p\_displayed\_trees.py} for the graph reconstruction and direct-pruning audit, \texttt{python3 src/verify\_k2p\_four\_leaf\_graft.py} for the exact $n=4$ regression, or \texttt{python3 verify.py} for every check; the latter ends with \texttt{ALL EXACT CHECKS PASSED}.
\end{document}
