...
9. **Scope and literature.** Check that the introduction and discussion do not
   ...
   taxon counts. In particular, independently compare arXiv:2607.12919v2 and
   v3, distinguishing operative formal results from roadmap prose, and assess
   the manuscript's account of what changed. Consult arXiv:2104.12479 and
   determine whether the stated simple/semisimple, nice, strict-level-two,
   leaf-range, model, and variety-level qualifications are accurate and
   whether that work affects the claimed novelty or priority. Do not presume
   the manuscript's answer. Verify other cited claims against primary sources
   when available and state exactly what you checked.
...
`RUN_REFEREE_REPLAY.sh`, `materials/strict_json.py`,
`materials/src/test_json_schema_mutations.py`,
`materials/src/test_k2p_semantic_mutations.py`,
`materials/src/test_k3p_semantic_mutations.py`, and
`materials/src/build_pdfs.sh`. Inspect all JSON certificates,
`materials/CERTIFICATE_FIELD_COVERAGE.md`, `PACKET_SHA256SUMS`, and
`PACKET_PROVENANCE.txt`. Treat the coverage inventory as an author-supplied
claim to audit, not as proof of its own completeness. Produce a claim-to-code
map and assess both mathematical coverage and orchestration integrity.
Determine independently:

- whether exact arithmetic is exact throughout and whether every algebraic
  root is uniquely and rigorously isolated;
- whether every operative certificate load uses the strict loader; whether it
  rejects duplicate keys at every depth, nonstandard numeric constants, and
  unregistered filenames; and whether its structural fingerprints bind keys,
  nesting, primitive types, lengths, and heterogeneous-array multiplicities;
- whether the hard-coded schema fingerprints can be independently recomputed
  from the canonical inputs, and whether embedded K3P sections and sidecars
  are compared without leaving a coordinated shadow-field route;
...
- whether the graph-based verifier genuinely reconstructs switchings and uses
  independent ordinary-state Markov pruning for both advertised exact
  witnesses;
- whether the K3P verifier binds the literal vertex/type/leaf-label schema and
  the complete ten-arc ID/parent/child/vector map before dictionary
  construction, and resolves every reticulation parent, child, and choice
  against the actual referenced arc;
- whether every stored compact-K2P transition row, including the composed
  root-suppressed `K_odot_K` row, is compared with an exact recomputation;
...
- whether unused certificate fields, untested branches, order dependence,
  ...
  create blind spots.
- whether the supplied K2P and K3P semantic-mutation tests alter fields,
  coupled labels, and descriptors in ways that would have survived mere
  self-consistency checks,
  and whether their expected failures test mathematical semantics rather than
  only schema or checksum enforcement.
...
claim. Also test hostile raw JSON, including a nested duplicate key, a
nonstandard numeric constant, an unknown operative field, and a fixed-length
heterogeneous-row-shape mutation. Confirm that the relevant verifier fails for
the substantive reason expected. Add other mutation tests suggested by your
code audit. Do not alter
