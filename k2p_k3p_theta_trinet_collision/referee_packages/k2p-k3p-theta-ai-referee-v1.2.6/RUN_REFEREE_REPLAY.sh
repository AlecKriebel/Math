...
printf '\nRunning focused strict-JSON/schema replays...\n'
(cd "$materials_dir" && python3 src/test_json_schema_mutations.py) \
  >"$work_dir/json-schema.normal.txt"
compare_output "Focused strict-JSON/schema replay" \
  "$materials_dir/verification_report_json_schema.txt" \
  "$work_dir/json-schema.normal.txt"

(cd "$materials_dir" && PYTHONOPTIMIZE=1 \
  python3 src/test_json_schema_mutations.py) \
  >"$work_dir/json-schema.optimized.txt"
compare_output "Optimized strict-JSON/schema replay" \
  "$materials_dir/verification_report_json_schema.txt" \
  "$work_dir/json-schema.optimized.txt"
...
(cd "$materials_dir" && python3 src/test_k2p_semantic_mutations.py) \
  >"$work_dir/k2p-semantic-mutations.txt"
