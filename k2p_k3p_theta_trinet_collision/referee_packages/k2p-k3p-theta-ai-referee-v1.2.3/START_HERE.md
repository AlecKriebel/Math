...
inputs for version 1.2.3 of *Exact Tree--Theta-Trinet Collisions under the
...
links, checks the exact packet path set, and verifies every listed SHA-256
digest. It runs the complete suite normally and with Python optimization,
compares focused outputs with the stored transcripts, regenerates the compact
K2P certificate in a disposable copy, and optionally rebuilds all PDFs in a
temporary directory. A passing replay is evidence about the supplied code and
data, not by itself proof that the implementation matches the manuscript; the
referee prompt requires that separate audit.
...
- `materials/verify*.py` and `materials/src/`: verifier and certificate
  generator source.
