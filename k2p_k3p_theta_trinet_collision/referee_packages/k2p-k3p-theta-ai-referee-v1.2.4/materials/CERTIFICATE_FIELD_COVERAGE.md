...
importantly, what it does not establish. It applies to version 1.2.4 of the
package.
...
source-edge reassignments, and unknown top-level fields. These negative tests
are in `src/test_k3p_semantic_mutations.py` and run as part of `verify.py` in
both normal and optimized replay modes.
...
- `manifest.sha256`, `FILE_SHA256SUMS`, and archive checksum sidecars detect
  path or byte changes relative to their listed values. When distributed with
  the files they cover and without an external signature, they establish
  self-consistency rather than external authentication.
