...
positive transition probabilities, and lies outside every globally
character-relabeled K2P parameter specialization. Its shared comparison-tree
distribution is openly identified as globally character-relabeled K2P. The
fixed theta map has rank 15, its local collision locus has dimension 23 and
codimension 6, and its fixed-output fibers have dimension 14. Removing the
three globally character-relabeled K2P tree strata gives a relatively open
dense set of nearby genuinely K3P shared distributions. The same conclusion
holds in the edgewise strictly continuous-time rate cone.
