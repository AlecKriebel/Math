...
Version 1.2.4 submission candidate. It is not a substitute for reading the
...
Let an enabled channel $y\to y'$ fire at $x=\rho+y$, where $\rho\geq0$, and
produce $x'=\rho+y'$. Weak reversibility gives a directed complex path

\[
  y'=z_0\to z_1\to\cdots\to z_m=y.
\]

For every path edge, the population $\rho+z_j$ contains its source $z_j$, so
the corresponding channel is enabled and takes $\rho+z_j$ to $\rho+z_{j+1}$.
...
positive-recurrence conclusion as a special case. In the published article,
Section 6 combines a tier criterion, a finite reaction word, and a
sampled-chain Foster argument. In the boundary case of Section 6.1, the
pure-species assumption first supplies either $S_v$ or $2S_v$. D-tier
maximality excludes $2S_v$, so $S_v$ is forced and its source propensity
supplies the required comparison. Exact internal numbering differs between
the arXiv and published versions, so the manuscript uses section-level
locators.
...
For residual population $\rho=x-t$, Stirling's formula gives

\[
  \sum_i\log(\rho_i!)
  =\sum_i(\rho_i\log \rho_i-\rho_i)
...
\[
  G_c(\rho)=\sum_i\bigl[\rho_i(\log(\rho_i/c_i)-1)+c_i\bigr].
\]
...
$\rho=x-t\geq0$. For a next channel $s\to u$,
...
a population return. Recurrent visits to one population state contribute an
almost surely divergent subseries of independent positive exponential holding
times, first establishing nonexplosion. A uniform positive lower bound on total
population rate then converts finite jump-count return into finite physical
return.
...
The regenerative occupation formula is an explicit representation, but no
claim is made of a closed-form or product-form stationary distribution,
...
One preserved historical audit also reports larger disposable stress runs for
which the scripts, complete inputs, and transcript were not retained. Those
counts are not reproducible release evidence and should not be credited as
such; the packaged verifier, tests, and canonical report are the reproducible
