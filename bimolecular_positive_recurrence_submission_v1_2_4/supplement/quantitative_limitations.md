...
For $d\ge2$, consider the unit-rate cycle on $d$ species

\[
 0\longrightarrow y_1\longrightarrow\cdots\longrightarrow y_d
 \longrightarrow0,
 \qquad y_j=2S_j.
\]
...
from zero, create $dN/2$ copies of $y_1$, pass the required pairs successively
...
\[
 ...
 \qquad T_N=1+G_N+(d-1)H_N,
\]

\[
 p_N=\frac{G_N}{T_N},\qquad
 \eta_N=\frac{\log G_N+(d-1)H_N\log(G_N/H_N)}{T_N}>0,
\]

and

\[
 \xi_N=-\frac{dH_N}{1+dH_N}\log H_N.
\]
...
\[
 D_0(z_N)=\eta_N\sum_{j=0}^{d-1}p_N^j+p_N^d\xi_N,
 \qquad
...
\[
 D_0(z_N)=-\frac{2}{d^d}\log N
 +O_d\!\left(\frac{\log N}{N}\right).
\]

Thus the present episode/Foster construction can produce a logarithmic
restoring coefficient that decays super-exponentially in the number $d+1$ of
