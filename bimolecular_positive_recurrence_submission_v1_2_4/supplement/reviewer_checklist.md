...
This checklist is intended for a skeptical expert reading of the Version 1.2.4
...
$\kappa_1n(2\log2-1)\to+\infty$. The initial expected reward at $(x_n,A)$
should be checked separately: the only enabled sources are $A$ and $A+B$,
and both give target/source ratio one, so the omitted initial term is exactly
zero. For the explicitly reachable carried target
