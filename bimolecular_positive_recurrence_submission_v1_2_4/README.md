...
## Version 1.2.4 submission candidate

This directory is the unrefereed Version 1.2.4 submission candidate frozen on
22 August 2026. It is prepared first for possible deposit in bioRxiv Systems
...
Version 1.2.4 is a separate, superseding patch release. Versions 0.3, 1.0,
1.1, 1.2, 1.2.1, 1.2.2, and 1.2.3 and their Git history remain unchanged. The
Version 1.2.3-to-1.2.4 relationship is recorded in
`preservation/VERSION_1.2.3_PROVENANCE.md`.
...
It supplies the regenerative occupation representation, but no closed-form or
product-form stationary distribution, moment or tail bound, mixing rate,
...
- `submission/biorxiv_upload_instructions.md`: ordered, human-only bioRxiv
  upload checklist.
- `submission/biorxiv_metadata.md`: copy-ready bioRxiv field metadata.
...
canonical report twice, and requires byte-identical outputs. Seven additional
release-tool tests reject cross-platform unsafe paths, unignored symbolic
links, and an absent, wrong, or lightweight release tag while permitting a
documented virtual environment. The
...
The scalar-envelope calibrations check the maximizer branch on both sides of
and at the threshold, together with exact pointwise monotonicity; the closed
form, limit, and finite backward composition are analytic steps in the paper.
The zero-length calibration checks the empty path-product convention, while
the required terminal ordinary jump is handled in the analytic episode
definition. The helper `Network` class requires at least one genuine channel
after reduction. An all-self-channel model bypasses that helper because
deleting its population-null channels leaves an absorbing minimal population
process, as treated directly in the manuscript.
...
clean checkout. The Git tag is the immutable commit identifier; before any
other check, the replay requires the literal Version 1.2.4 tag, requires it to
be annotated, and requires it to identify the checked-out commit. It then
prints the resolved commit, tool versions, report/PDF/archive digests, and
requires a clean package tree.

## Package map

- `audit/`: preserved earlier audits and the focused Version 1.2.4 editorial
  and release audit.
...
Generative-AI systems were used materially from 5--22 August 2026. The
