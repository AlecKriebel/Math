...
named there need not be present in the current Version 1.2.4 directory.

## Version 1.2.4 editorial-clarity and release-hardening patch — 19--22 August 2026

Version 1.2.4 is a separate patch-release directory and supersedes Version
1.2.3 for preprint and journal use. It preserves the Version 1.2.3 directory,
tag, PDFs, archive, validation record, theorem, and standalone verifier.

- Distinguished the displayed regenerative occupation representation from a
  closed-form or product-form stationary distribution.
- Added a forward pointer to the proof of Corollary 2.3 and stated why the
  initial expected reward in the Anderson--Cappelletti--Kim episode is zero.
- Defined the active complex set explicitly, qualified the deterministic
  motivation by strictly positive initial conditions, removed “candidate” from
  the bioRxiv PDF metadata, and compressed a repeated limitations sentence.
- Rechecked the current Xu and ConStRAINeD primary records through 22 August 2026.
  Retained Xu's official Version 2 title and rejected the proposed replacement,
  which is the superseded Version 1 title.
- Adjudicated an independently prompted AI-referee report. Added fail-closed
  exact annotated-tag enforcement and regression tests for missing, wrong, and
  lightweight tags; clarified non-load-bearing verifier scope and historical
  evidence without changing verifier logic.
- Added a direct chemical-master-equation/biochemical-model opening to the
  preprint abstract and updated current literature and AI-use dates through 22
...
Version 1.2.3 is a separate patch-release directory and supersedes Version
1.2.2 for preprint and journal use. It preserves the Version 1.2.2 directory,
tag, PDFs, archive, validation record, theorem, and standalone verifier.

- Defined the exact Anderson--Cappelletti--Kim entropy-like function in the
  boundary-example remark.
...
## Version 1.2.2 submission-status patch — 16 August 2026

Version 1.2.2 is a separate patch-release directory and supersedes Version
1.2.1 for preprint and journal use. It leaves the Version 1.2.1 directory,
tag, PDFs, archive, validation record, theorem, proof, and standalone verifier
unchanged.

- Updated the current literature-status paragraph and bibliography note to
  state both facts shown by the live ConStRAINeD page: the overview describes
  a complete two-dimensional proof and sketches its approach, while item 16
  lists the five-author manuscript as in preparation. No public manuscript
  was located in the accompanying 16 August 2026 check.
- Added an ordered, human-only bioRxiv upload checklist and refreshed the
  submission metadata for the current DOI, funder, author-consent, screening,
  and final-approval workflow.
- Rebuilt and revalidated the outer manuscript, package, and public artifacts;
  the Version 1.2.0 verifier and canonical report remain byte-for-byte
  unchanged.
