#!/usr/bin/env python3
"""Clean-room hostile audit of the compact DoubleLex LRAT publication package.
...
def audit_target_inventory() -> dict[str, object]:
    ...
    require(
        snapshot["total_size_bytes"] == 64_301_640,
...
def audit_manifest() -> dict[str, object]:
    ...
    require(
        manifest["status"] == "PENDING_INDEPENDENT_PUBLICATION_PACKAGE_REVIEW",
        "publication manifest status is not the frozen pending-review value",
    )
    require(
    ...
    expected_files = {
        ...
        "verifier": {
            ...
            "size_bytes": TARGET_FILES["verify_publication.py"][0],
            "sha256": TARGET_FILES["verify_publication.py"][1],
        },
        "compressed_lrat": {
...
def validate_successful_verifier(run: dict[str, object]) -> dict[str, object]:
    ...
    require(
        payload["schema"]
        == "gamma-theta-doublelex-publication-verifier-v1"
        and payload["verdict"]
        == "VERIFIED_EXACT_DOUBLELEX_CNF_UNSAT_ONLY",
...
def metadata_mutation_observations(
    private_campaign: Path,
) -> list[dict[str, object]]:
    ...
    payload_out = validate_successful_verifier(run)
    observations.append(
        {
            "label": "publication_manifest_schema_version_one_byte",
            "expected_for_whole_package_integrity": "REJECT",
            "observed_exit_code": run["exit_code"],
            "observed_verdict": payload_out["verdict"],
            "passed_mathematical_core_despite_metadata_mutation": True,
            "whole_package_fail_closed": False,
        }
    )
...
def audit_private_verifier(work: Path) -> dict[str, object]:
    ...
    metadata_observations = metadata_mutation_observations(private_campaign)
    return {
        ...
        "metadata_mutation_observations": metadata_observations,
    }
...
def main() -> int:
    ...
    evidence = {
        "schema": (
            "gamma-theta-order12-k4-doublelex-publication-hostile-evidence-v1"
        ),
        "schema_version": 1,
        "started_unix_ns": started_ns,
        "finished_unix_ns": time.time_ns(),
        ...
        "verdict": (
            "ACCEPT_EXACT_DOUBLELEX_CNF_UNSAT_PUBLICATION_PAYLOAD_ONLY_"
            "WITH_METADATA_INTEGRITY_LIMITATION"
        ),
        "defect": {
            "severity": "NONBLOCKING_FOR_EXACT_CNF_UNSAT_CORE",
            "description": (
                "verify_publication.py does not read publication-manifest.json "
                "or bind README.md. One-byte mutations of either still produce "
                "the narrow exact-CNF success verdict. All decisive formula, "
                "proof, checker, author-certificate, and hostile-review "
                "mutations tested were rejected fail-closed."
            ),
        },
    }
