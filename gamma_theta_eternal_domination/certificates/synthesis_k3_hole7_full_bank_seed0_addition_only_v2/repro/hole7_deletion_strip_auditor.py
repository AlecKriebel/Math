...
SCHEMA = "gamma-theta-hole7-addition-only-recovery-v2"
ORDER = 12
VARIABLE_COUNT = 6886
CLAUSE_COUNT = 21718
...
STRICT_TIME_PATTERN = re.compile(
...
def _stable_checker_stdout(stdout: bytes) -> bytes:
    """Remove only DRAT-trim's inherently nondeterministic elapsed-time value."""
    normalized = stdout.replace(b"\r", b"")
    matches = STRICT_TIME_PATTERN.findall(normalized)
    require(
        len(matches) == 1,
        "strict checker lacks exactly one canonical verification-time line",
    )
    try:
        elapsed = float(matches[0].decode("ascii"))
    except (UnicodeDecodeError, ValueError) as error:
        raise AuditFailure("strict checker verification time is malformed") from error
    require(0.0 <= elapsed <= 620.0, "strict checker verification time is invalid")
    return STRICT_TIME_PATTERN.sub(
...
def _checker_verified(stdout: bytes, stderr: bytes) -> dict[str, object]:
    ...
    stable_stdout = _stable_checker_stdout(stdout)
    return {
...
the core.  Fresh replay compares the complete checker transcript after
normalizing only its elapsed-time number; all logical counters and verdict
text must still match exactly.  Thus the retained proof is a checked RUP-only
refutation of the exact full-bank `hole7` formula.
...
def audit_certificate(
    ...
    if replay_checker:
        ...
        require(
            _stable_checker_stdout(fresh_stdout)
            == _stable_checker_stdout(saved_stdout)
            and fresh_stderr == saved_stderr,
            "fresh strict replay transcript differs beyond elapsed time",
        )
        for key in (
            "exit_code",
