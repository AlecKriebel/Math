...
\title{An explicit 44-variable vanishing witness\\
from a 22-variable cubic Keller map}
\author{Alec Kriebel\\[2pt]\large with heavy assistance from ChatGPT 5.6 Sol}
\date{First public release: 21 July 2026, 14:42:57 UTC\\
Priority correction: 21 July 2026, 15:20:23 UTC\\
Canonical consolidated version: 21 July 2026}

\begin{document}
\maketitle
\begin{abstract}
Starting from the recently announced three-dimensional noninjective Keller
map, a rank-compressed homogenization of a 13-variable stable model gives a
22-variable cubic homogeneous noninjective Keller map. The de Bondt--van den
Essen symmetric reduction then gives a 44-variable homogeneous quartic
Hessian-nilpotent polynomial with 538 monomials and an exact collision.
Expanded potentials and collisions are supplied as machine-readable exact
certificates. For reuse we also record a normalized six-variable gradient
lift. A post-release priority audit found that Cassidy had already executed
the equivalent six-dimensional symmetric transport; no novelty is claimed
for that construction.
An appendix preserves the uniform rational collision from the archived first
exploration while crediting the earlier, stronger monodromy theorem for the
full weighted-lift family.
\end{abstract}
...
six-variable identity linear part and both finite fibers. A separate Node.js
checker independently implements exact rational complex arithmetic with
JavaScript \texttt{BigInt} and repeats the exported-certificate checks. Neither
certificate checker is a stand-alone proof of Hessian nilpotence.

The six-variable result is an explicit instance of Meng's known reduction,
not a new reduction theorem. Cassidy's public repository
commit~\cite{cassidy}, authored at 14:46:10 UTC on 20 July 2026, had already
executed the equivalent de Bondt--van den Essen/Meng transport in $\C^6$,
checked symmetry, and transported the same three-point fiber. Our
determinant-one, identity-linear formula is a normalized presentation, not an
independent discovery. The first version of this note failed to find that
artifact and overstated this part of its novelty claim.

Zhang~\cite{zhang} left the Vanishing Conjecture consequence existential. Our
earlier note~\cite{kriebel54} gave a 54-variable certificate.
That note is now an archival derivation subsumed by the present paper.
Thompson~\cite{thompson} posted a 24-variable cubic homogeneous reduction at
03:29:42 UTC on 21 July 2026. His work contains the same rank-compression
principle in executed form and has priority for that idea. Thompson's map has
24 variables, 54 cubic monomials, and 23 active nonlinear coordinates; ours
has 22 variables, 72 monomials, and 21 active nonlinear coordinates. Both are
over $\Q$ with rational collisions. Thus ours is smaller only in ambient
dimension, while Thompson's is sparser. The narrower candidate contribution
is the different $13+8+1=22$ construction and its 44-variable quartic.
...
quartic certificate. This is not an exhaustive claim of worldwide priority.
The literature is changing by the hour and all novelty statements are
provisional.

\appendix
\section{A uniform rational collision in a weighted-lift specialization}
...
Gallagher introduced the weighted-lift family~\cite{gallagher}, and Mikhail
Szh and Fable~\cite{mikhail} publicly proved full $S_n$ monodromy for the
entire family before Exploration 01 appeared. No monodromy or deck-group
...
x\gamma
\right).
\label{eq:weighted-specialization}
\end{equation}
...
\]
For $r\in\{1,2\}$, let $g_r$ denote the corresponding displayed value and set
\begin{equation}
X_{n,r}=\left(
\frac1{g_r},\ r-g_r,\
...
and~\eqref{eq:weighted-collision} in exact rational arithmetic. This formula
is a compact reusable certificate inside the broader Gallagher--Mikhail
family, not an independent theorem.
...
\bibitem{cassidy}
E. Cassidy, THM-1430: An explicit symmetric-case Keller counterexample on
$\C^6$, public GitHub repository, commit
\texttt{40e1e20f9ee113245f8e4e4b22ecd798fa1ffbfc}, 20 July 2026;
\url{https://github.com/eliottcassidy2000/math/blob/40e1e20f9ee113245f8e4e4b22ecd798fa1ffbfc/01-canon/theorems/THM-1430-explicit-symmetric-keller-counterexample-on-C6.md}.
\bibitem{kriebel54}
...
Conjecture in dimension three: every generic fiber degree $n\ge3$ occurs,
weighted-lift construction, 20 July 2026;
\url{https://jacobianfun.org/}.
\bibitem{mikhail}
Mikhail Szh and Claude Fable, The Antiderivative Resolvent of the Weighted-Lift
Family of Keller Maps, initial public commit
\texttt{f8a6d6794febb551050d73b8cf6ffab9da52d047}, 21 July 2026;
...
21 July 2026; \href{https://aleckriebel.github.io/Math/papers/symmetric-keller-and-vanishing/}{project page}.
\end{document}
