...
\[
 ...
 =\ell_m^T\Delta r_m<0.
\]
Together with $\widetilde\ell_m(L)^Tr_m<0$, this gives
\[
 \eta_m(L)=
 \frac{\widetilde\ell_m(L)^T\mathsf H_m(L)\Delta r_m}
 {\widetilde\ell_m(L)^Tr_m}>0.
\]
Moreover,
\[
 \ker\!\left[\mathsf H_m(L)(A_m-\Delta)\right]
 =\ker(A_m-\Delta)=\operatorname{span}\{r_m\},
\]
because $\mathsf H_m(L)$ is invertible and the unit critical zero is simple.
If $\mathsf H_m(L)(A_m-\Delta)v=r_m$ for a generalized eigenvector $v$, then
left multiplication by $\widetilde\ell_m(L)^T$ would imply
$0=\widetilde\ell_m(L)^Tr_m$, a contradiction.  Hence the scaled critical
...
\[
 ...
 d_Z=\frac{2-2p-4q}{q}.
\]
The near-threshold affine path is
\[
p=\varepsilon,\quad
