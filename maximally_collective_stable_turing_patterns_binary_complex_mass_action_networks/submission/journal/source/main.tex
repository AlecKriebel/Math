...
\hypersetup{
 pdftitle={Maximally Collective Stable Turing Patterns in Binary-Complex Mass-Action Networks},
 pdfauthor={Alec Kriebel},
 pdfsubject={Exact diffusion design and exponent-optimal heterogeneity trade-offs},
...
\title{Maximally Collective Stable Turing Patterns\\
\large in Binary-Complex Mass-Action Networks\\[0.25em]
\normalsize Exact Diffusion Design and Exponent-Optimal Heterogeneity Trade-Offs}
...
subsystem responsible for patterning.  We construct, for every $n\ge4$, an
$n$-species classical mass-action topology whose source and product complexes
contain at most two molecules and whose instability is maximally collective.
Uniformly over every positive-equilibrium realization of the topology, every
principal subsystem below order $n-1$ is Hurwitz, whereas one order-$(n-1)$ subsystem
has a positive real eigenvalue.  We then derive an exact diffusion-design law:
for any homogeneously stable realization, stationary destabilization along a
positive diagonal diffusion ray occurs if and only if
$d_Z>8h_Z\sum_{j=2}^{n-2}d_j/h_j$.  This yields sharp fixed-equilibrium
contrast infima and the universal trade-off
$\chi_D\chi_H>8(n-3)$.  At unit equilibrium we give a rational primary
supercritical Turing realization with
$\chi_D=23(91n-274)/63$ and locally exponentially attracting positive
patterns.  A second exact family redistributes heterogeneity between diffusion
and equilibrium concentrations; both contrasts are $\Theta(\sqrt n)$, which
matches the universal lower bound in exponent.  The proof combines a complete
positive-realization parametrization, an exhaustive strongly connected block
classification, a one-bad-minor matrix theorem, conservation-compatible
Lyapunov--Schmidt reduction, and exact symbolic sign certificates.  The
networks are synthetic, and the bifurcation and stability conclusions are
local for each fixed dimension.
\end{abstract}
...
negative, and the remaining omission minors are positive or zero.  This
``one-bad-minor'' structure reduces stationary diffusion design to one strict
...
\item a complete two-flux-parameter family of all positive-equilibrium
Jacobians;
...
$\chi_D,\chi_H=\Theta(\sqrt m)$, matching the universal minimax lower bound in
exponent.
\end{enumerate}
...
\caption{The synthetic topology $\widehat N_m$.  The shaded principal species
set $\cC_m=\{X_1,\ldots,X_m\}$ is the unique order-$(n-1)$ block with negative
signed determinant.  The separate $Z$ boundary pair controls the exact
diffusion law.}
\label{fig:network}
\end{figure}
...
balance equates the final reversible pair.  The two displayed kernel vectors
are independent.  Deleting one reaction from each circuit leaves an explicit
$m\times m$ triangular stoichiometric minor with nonzero diagonal, so the rank
is $m$.  The image and positive-cone statements follow.
\end{proof}
...
For every proper principal species set of cardinality less than $m$, each
nontrivial strongly connected diagonal block of the induced Jacobian graph is
one of:
\begin{enumerate}[leftmargin=2em]
\item the long cycle on $X_1,\ldots,X_{m-1}$;
\item the long cycle on $X_2,\ldots,X_m$;
\item a principal block of the boundary triad $\{X_1,X_m,Z\}$;
\item a negative singleton.
\end{enumerate}
All remaining induced matrices are block triangular combinations of these.
On the hypersurface $b=2a$, the edge $X_1\to X_m$ disappears and only refines
this decomposition.
\end{lemma}

\begin{proof}
Interior chain edges point forward.  A return path through the lower boundary
requires either the complete $X_1,\ldots,X_{m-1}$ circuit or the complete
$X_2,\ldots,X_m$ circuit.  Once one long cycle is present, adding a further
boundary vertex requires at least $m$ species.  Without a complete long cycle,
only the three boundary vertices can form a nontrivial strongly connected
component.  Every other retained vertex has only its negative diagonal inside
its component.  Removing an edge at $b=2a$ cannot merge components.
\end{proof}
...
sets follows from \cref{lem:scc} and Frobenius block triangularization.
...
\section{A one-bad-minor diffusion theorem}

For $I\subseteq[n]$, put
$a_I=(-1)^{|I|}\det J_{I,I}$ and use the convention $a_\varnothing=1$.

\begin{theorem}[One-bad-minor diffusion ray]\label{thm:onebad}
Let $J\in\R^{n\times n}$ have one semisimple zero eigenvalue and otherwise
stable spectrum.  Assume every principal block of order at most $n-2$ is
Hurwitz and, among the signed order-$(n-1)$ principal minors, at most one is
negative.  For $D=\diag(d_1,\ldots,d_n)\succ0$, define
$p_D(s)=\det(sD-J)$.  Then
...
\begin{proof}
The principal-minor expansion of $\det(sD-J)$ gives
\cref{eq:pdbeta,eq:betak}; the constant term vanishes because $\det J=0$.
Every $a_I$ of order at most $n-2$ is positive, hence $\beta_k>0$ for
$k\ge2$.  The bracket in \eqref{eq:pdbeta} is strictly increasing for
$s>0$, proving existence, uniqueness, signs, and scalar simplicity of $s_*$.
Since
$p_D(s)=\det(D)\det(sI-D^{-1}J)$, scalar simplicity is algebraic simplicity of
the zero eigenvalue.
...
\begin{equation}\label{eq:chis}
 ...
 =\sum_Ia_I\prod_{j\notin I}(\lambda+sd_j).
\end{equation}
All derivative contributions from $|I|\le n-2$ are nonnegative, with a
strictly positive contribution.  The total derivative contribution from
$|I|=n-1$ is
$\sum_{|I|=n-1}a_I=\beta_1(\1)>0$: equal diagonal damping shifts every stable
nonzero eigenvalue and the simple zero strictly left, so the coefficient of
$s$ in $\det(sI-J)$ is positive.  Thus $\chi_s'(\lambda)>0$ for
$\lambda\ge0$.  Since $\chi_s(0)=p_D(s)$ and $\chi_s(\lambda)\to\infty$, the
positive-real-root equivalence \eqref{eq:exactband} follows.
\end{proof}

\section{Exact diffusion design and sharp linear contrast}

The all-spectrum theorem supplies the lower-order hypotheses of
\cref{thm:onebad}.  The remaining information is the complete order-$m$
omission table.
...
\begin{proof}
Omitting $Z$ yields the chain expansion \eqref{eq:coreminor}.  Omitting an
interior $X_j$ breaks the chain into negative singleton factors and the
remaining boundary triad contributes signed determinant $16a^2b$.
Omitting $X_1$ leaves the restricted conservation vector as a left nullvector;
omitting $X_m$ leaves the restriction of the full right kernel vector, whose
$X_m$ coordinate is zero.  Multiplication by the omitted diffusion coefficient
and summation gives \eqref{eq:betaone}.  The coefficient $8$ is the exact
ratio $16/2$ between the positive interior-omission weight and the magnitude
of the unique negative $Z$-omission weight.
\end{proof}

\begin{theorem}[Exact stationary diffusion law]\label{thm:diffusionlaw}
Assume $J_m(a,b,H)$ is stable on its stoichiometric subspace.  A nonzero
stationary crossing occurs on the positive ray $sD$ if and only if
\begin{equation}\label{eq:criterion}
 \boxed{\quad d_Z>8h_Z\sum_{j=2}^{m-1}\frac{d_j}{h_j}.\quad}
\end{equation}
When \eqref{eq:criterion} holds, the threshold $s_*(H,D)$ is unique and simple,
and the exact positive-real-eigenvalue interval is $0<s<s_*$.  Scaling
$D\mapsto\gamma D$ sends $s_*\mapsto s_*/\gamma$.
\end{theorem}

\begin{proof}
Combine \cref{thm:onebad,prop:omissions}.  A second derivation uses the left vector $c$ and the right zero vector
\[
...
For a Neumann interval $(0,L)$, mode $k$ has $s=q_k^2=(k\pi/L)^2$.
Therefore every mode with $q_k^2<s_*$ has a positive real eigenvalue, and no
mode with $q_k^2>s_*$ has one.  A separate spectral certificate is required to
...
\begin{equation}\label{eq:unitinf}
 \inf\chi_D=8(m-2).
\end{equation}
Every stationary crossing satisfies the universal strict trade-off
\begin{equation}\label{eq:productlower}
 \boxed{\quad \chi_D\chi_H>8(m-2).\quad}
\end{equation}
The product lower bound is sharp as an infimum over stable realizations.
\end{theorem}
...
$T(H)\ge8(m-2)/\chi_H$, yielding \eqref{eq:productlower}.  Taking $H=I$ and
the approaching diffusion sequence shows sharpness of the product infimum.
...
\item a conservation-compatible Lyapunov--Schmidt reduction has
...
$R=5\lambda^2+33\lambda+16$.  The exact difference
$|1+\lambda|^2|P|^2-|R|^2$ has 35 nonnegative monomials in
$\Re\lambda$ and $(\Im\lambda)^2$, with equality only at the origin.
...
$(\lambda,t)=(0,1)$.  This proves the complete primary mode statement.
An exact left vector is
...
\end{equation}
Its numerator reduces to $R_m+C_m\mathcal H_m$ with
$\mathcal H_m=\sum_{j=1}^{m-2}K_j^{-1}$.  The exact shifted-polynomial
certificate proves the numerator positive and the denominator negative for
all $m\ge3$, hence $c_m<0$.

Crandall--Rabinowitz gives branches
$A_\pm(\mu)=\pm\sqrt{-\eta_m\mu/c_m}+O(\mu^{3/2})$.
The center eigenvalue is $-2\eta_m\mu+O(\mu^2)<0$; the complementary gap
persists.  On the real fixed-mass $L^2$ space, positive Neumann diffusion with
domain $H_N^2$ is sectorial, while in one dimension the quadratic Nemytskii
map is smooth from $H^1$ to $L^2$.  The principle of linearized stability
then gives local exponential asymptotic stability in $H^1$
\cite{CrandallRabinowitz1971,Kato1995,Henry1981,Kielhoefer2012}.  The complete
coefficient tables and algebra appear in the Supplement.
\end{proof}

\section{A stable diffusion--equilibrium trade-off family}

Put $r=m-2$ and retain the effective diffusion matrix $\Delta_m$ from
\eqref{eq:Dimp}.  Define
\begin{equation}\label{eq:Lrange}
 L_0(r)=\frac1{\sqrt{3r}},\qquad
 L_1(r)=\frac{90r}{90r+1}.
\end{equation}
For $L\in[L_0,L_1]$, set
...
For every $m\ge3$ and $L\in[L_0(r),L_1(r)]$, the physical realization
\eqref{eq:HL}--\eqref{eq:Dphys} has a primary simple transverse first-mode
stationary bifurcation, a negative cubic coefficient, and two nearby positive
locally exponentially asymptotically stable patterned equilibria.  Its
physical contrasts are
\begin{align}
 \chi_D(L)&=\frac{23}{63}\,91rL,\label{eq:chiDpareto}\\
 \chi_H(L)&=\frac{91r-1}{91rL},\label{eq:chiHpareto}\\
 \chi_D(L)\chi_H(L)&=\frac{23(91r-1)}{63}.\label{eq:productpareto}
\end{align}
At $L=L_0$,
...
\end{equation}
the simultaneous exponent $1/2$ is globally optimal.  No constant-optimality
or complete global Pareto-frontier claim is made.
\end{theorem}

\begin{proof}[Proof architecture]
In normalized concentrations $z=H_m(L)x$, the PDE is
\begin{equation}\label{eq:normalizedPDE}
 z_t=H_m(L)\{f_m(z)+\Delta_m z_{\xi\xi}\}.
\end{equation}
The stationary bracket $f_m(z)+\Delta_mz_{\xi\xi}$ and its critical right
vector are unchanged.  The dynamic left vector is
$q_m(L)=H_m(L)^{-1}\ell_m$, and the physical fixed-mass vector becomes
$H_m(L)^{-1}c$.
...
\[
 ...
 \left(1+2rLx+\frac13y^2\right),
\]
because $rL^2\ge1/3$.  The corresponding boundary difference has 84
nonnegative monomials and equality only at $(\lambda,t)=(0,1)$.  A separate
34-term certificate proves homogeneous relative stability.  Hence the primary
mode and all higher-mode conclusions hold throughout the certified interval.

The second harmonic is unchanged by the invertible row factor.  The
homogeneous correction changes by a kernel gauge:
...
$N_m(L)>1/200$.  Meanwhile
\[
 \ell_m^TH_m(L)^{-1}r_m
 =-\frac{485873}{924210}-\frac{11180}{1467}L(m-2)<0.
\]
Therefore $\eta_m(L)>0$ and $c_m(L)<0$.  The branch and stability proof is the
same sectorial exchange-of-stability argument as in \cref{thm:unitstable}.
The contrast formulas follow by comparing the explicit entries of
$H_m(L)$ and $D_m^{\rm phys}(L)$ on the interval \eqref{eq:Lrange}.
...
dashed hyperbolas are the universal necessary bounds
$\chi_D\chi_H=8(m-2)$; the solid curves are certified stable families with
product $23(91(m-2)-1)/63$.  The marked point is the square-root-balanced
endpoint.  Constants are not claimed optimal.}
\label{fig:tradeoff}
\end{figure}
...
\begin{equation}\label{eq:frontierbounds}
 ...
 \le\frac{23(91m-183)}{63}.
\end{equation}
A canonical near-threshold affine path has, at $m=3$,
...
\centering
\small
\input{data/contrast_table.tex}
\caption{Exact-design diagnostics.  $\chi_D^{\rm unit}$ is
\eqref{eq:improvedcontrast}; the balanced values use $L=L_0$ in
\eqref{eq:balanced}.  The normal-form values are for the unit-equilibrium
stable profile.  The table illustrates exact formulas but is not used to prove
any all-dimensional statement.}
\label{tab:finite}
\end{table}

\begin{proposition}[Retuned local robustness]\label{prop:robust}
Fix $m$ and a certified $L$.  Sufficiently small perturbations of positive
rate constants, equilibrium coordinates, diffusion ratios, and interval
length admit a unique nearby scalar diffusion multiplier at which the primary
supercritical bifurcation and local exponential stability persist.
\end{proposition}
...
all high modes.  The Lyapunov--Schmidt coefficients and branch spectral
...
branches.  The right panel compares the measured adjoint-projected amplitude
...
The possibility of order-$(n-1)$ responsible subsystems for general fixed
matrices is established in multicomponent Turing theory
\cite{Satnoianu2000,Anma2012}; that fixed-matrix possibility is not claimed as
new.  The present distinction is the conjunction of one indexed
...
result instead gives one exact infinite topology family with a universal
over-realizations theorem.  Parameter-rich unstable cores allow derivative
freedoms absent from classical mass action \cite{VassenaStadler2024}.
Exact fixed-three-species criteria address a different, fixed-Jacobian scope
\cite{Piskovsky2025}.  Biochemical screens study interpretable reaction
...
explicit two-parameter Jacobian image makes the topology-wide quantifier
tractable.  The long chain forces every unstable principal subsystem to
contain almost all species, while the boundary triad remains Hurwitz under
...
the exact diffusion cost is computable; stable positive patterns occur; and
diffusion and equilibrium contrasts can be balanced at the globally optimal
square-root exponent.  The remaining constant gap is explicit rather than
...
figure scripts, and a one-command replay accompany the manuscript.
