...
\caption{The synthetic topology $\widehat N_m$.  The dashed outline marks the
principal species set $\cC_m=\{X_1,\ldots,X_m\}$, the unique order-$(n-1)$ block with negative
...
\begin{proof}
Combine Theorem~\ref{thm:diffusionray} and
Proposition~\ref{prop:omissions}.  A second derivation uses the left vector $c$ and the right zero vector
\[
...
Therefore every nonzero mode with $0<q_k^2<s_*$ has a positive real eigenvalue, and no
...
patterned equilibria that are locally exponentially asymptotically stable in
the fixed-integrated-mass $H^1$ phase space.  Its
...
$|1+L\lambda|$, because, with $r_i=K_{i-1}/K_i>1$,
\[
 |1+Lr_i\lambda|^2-|1+L\lambda|^2
 =2L(r_i-1)x+L^2(r_i^2-1)(x^2+y^2)\ge0.
\]
If $\nu\ge2$, then
\[
 |Q|^2\ge |1+L\lambda|^{2\nu}
...
simple.  For $\nu=1$, putting $c=91L/90$ gives
\[
 \frac{QF-R}{\lambda}
 =c\lambda^3+(1+11c)\lambda^2+(6+31c)\lambda+(16c-2).
\]
Here $c>1/8$ and the remaining cubic Routh--Hurwitz determinant is
$6+99c+325c^2>0$.  Hence the primary mode and all higher-mode conclusions
hold throughout the certified interval.  The interval is nonempty: the case
...
dotted hyperbolas are the universal necessary bounds
$\chi_D\chi_H=8(m-2)$; the solid curves are certified stable families with
product $23(91(m-2)-1)/63$.  Each marked point is the certified
...
figure scripts, and replay instructions are available at
\url{https://github.com/AlecKriebel/Math/tree/main/maximally_collective_stable_turing_patterns_binary_complex_mass_action_networks}.
