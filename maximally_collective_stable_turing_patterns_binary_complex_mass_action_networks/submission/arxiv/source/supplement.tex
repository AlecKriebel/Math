...
\[
 \frac{\det(\lambda I-H A_m)}{\det H}=Q(\lambda)F(\lambda)-R(\lambda),
\]
where
\[
...
\[
 \left(1+Ax+\frac54(\Im\lambda)^2\right)|F|^2-|R|^2
\]
...
right half-plane.  Moreover
\[
 (QF-R)'(0)=16L\sum_{i=2}^{m-1}\frac{K_{i-1}}{K_i}-2>0,
\]
...
corresponding polynomial.  In the Pareto spatial table, coefficient entries
are polynomials in $A=2\nu L$.  In the Pareto homogeneous table, they are
polynomials in $U=A-1/4$.  All listed coefficients are nonnegative; the
strict rows identify the unique equality case.  From the repository root,
the command
