...
\hypersetup{pdftitle={Technical Supplement: Maximally Collective Stable Turing Patterns in Binary-Complex Mass-Action Networks},pdfauthor={Alec Kriebel}}
...
\large Maximally Collective Stable Turing Patterns in Binary-Complex Mass-Action Networks}
\author{Alec Kriebel}
\date{August 2026}
\begin{document}
\maketitle
...
The induced directed graph has the forward chain
$X_2\to X_3\to\cdots\to X_{m-1}$, boundary arrows involving
$X_1,X_m,Z$, and no alternative route that can close an interior cycle.
A nontrivial strongly connected induced block with fewer than $m$ vertices is
therefore one of the two complete long cycles, a boundary-triad block, or a
negative singleton.  At $b=2a$, the coefficient $A_{m1}$ vanishes; deleting
that edge can only split a component and cannot create a new component.
...
are strict because $a+b>a$ and $4a+b>4a$.  This proves Hurwitz stability of
every smaller principal block.
...
\section{The one-bad-minor theorem in full detail}

Let $a_I=(-1)^{|I|}\det J_{I,I}$.  Multilinearity in the diagonal entries of
$sD$ gives
\[
...
\]
For $k\ge2$, every contributing principal block has order at most $n-2$ and
is Hurwitz, so $a_I>0$.  Hence $\beta_k>0$.
...
$\sum_{|I|=n-1}a_I$.  This equals the coefficient of $s$ in
$\det(sI-J)$ and is positive because equal damping sends the simple zero to
$-s$ and shifts each stable eigenvalue by $-s$.  Thus $\chi_s'>0$ on the
...
For $J=A_m(a,b)H$, omitting $Z$ gives
\[
 (-1)^m\det J_{\widehat Z,
 \widehat Z}=-2a^{m-1}b\prod_{i=1}^mh_i.
\]
For $2\le j\le m-1$, omitting $X_j$ breaks the chain.  Each resulting
one-way component contributes a negative diagonal factor, while the surviving
boundary component has signed determinant $16a^2b$.  Therefore
\[
 (-1)^m\det J_{\widehat{X_j},\widehat{X_j}}
 =16a^{m-1}bh_Z\prod_{i\ne j}h_i.
\]
Omitting $X_1$ leaves the restricted conservation vector in the left kernel.
Omitting $X_m$ leaves the restriction of
$H^{-1}(2,-2,\ldots,-2,0,1)^T$ in the right kernel.  Hence both determinants
vanish.
...
For fixed $H$, if $d_{\min}=\min_i d_i$, then
\[
 d_Z>8h_Z\sum_{j=2}^{m-1}\frac{d_j}{h_j}
 \ge d_{\min}T(H),
\]
so $\chi_D>T(H)$.  Setting all $X$ diffusivities to one and
$d_Z=T(H)+\varepsilon$ proves sharpness as a nonattained infimum.  Since
$h_Z/h_j\ge1/\chi_H$, one obtains
$\chi_D\chi_H>8(m-2)$.
...
Define
\[
 H_m=\sum_{j=1}^{m-2}\frac1{91m-181-j}.
\]
Then
\[
 \ell^Tr=-\frac{7043400m-13600927-7043400H_m}{924210}<0,
\]
\[
 \ell^TD_mr=-\frac{2(1760850H_m+16559)}{462105}<0.
\]
The harmonic bound $H_m\le(m-2)/(90m-179)$ makes the first sign an elementary
...
\end{align*}
Here the fourth line holds for $3\le i\le m-1$.  Reaction-wise conservation
implies $c^TB(u,v)=0$.
...
\]
The numerator equals $R_m+C_mH_m$, where $R_m>0$ and $C_m<0$.  Substitution of
$H_m\le(m-2)/(90m-179)$ reduces positivity to the degree-five polynomial
\begin{align*}
...
Let $r=m-2$, $L\in[1/\sqrt{3r},90r/(90r+1)]$, and
\[
...
\[
 ...
 \left(1+2rL\Re\lambda+\frac13(\Im\lambda)^2\right).
\]
The 34- and 84-term coefficient tables in Section~S8 prove homogeneous and
all-mode stability.

The physical zero-mode gauge is
\[
 w_0(L)=w_0^{\rm ref}+\tau_m(L)\rho,
 \qquad
 \tau_m(L)=-\frac{(H^{-1}c)^Tw_0^{\rm ref}}{(H^{-1}c)^T\rho}.
\]
...
The contrasts are
\[
 \chi_D(L)=\frac{23}{63}(91rL),\qquad
 \chi_H(L)=\frac{91r-1}{91rL},
\]
with constant product $23(91r-1)/63$.  At $L=1/\sqrt{3r}$, both have
square-root growth.
...
polynomial in $A=2rL\Re\lambda$ (or any positive lower coefficient replacing
...
all-spectrum block theorem, omission minors, one-bad-minor theorem, diffusion
