...
For every positive realization and every principal species set $I$ with $0<|I|<m$, $J_{I,I}$ is Hurwitz. Every nontrivial strongly connected diagonal block is one of two stable long cycles, a principal block of the boundary triad $\{X_1,X_m,Z\}$, or a negative singleton. The triad satisfies the cubic Routh--Hurwitz inequalities coefficientwise.
...
$\widetilde\ell_m(L)=\mathsf H_m(L)^{-1}\ell_m$ satisfies
\[
 \widetilde\ell_m(L)^T\mathsf H_m(L)\Delta_mr_m
 =\ell_m^T\Delta_mr_m<0,
 \qquad \widetilde\ell_m(L)^Tr_m<0,
\]
so the crossing coefficient is positive.  A
