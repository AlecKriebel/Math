...
D^{\rm phys}=H\Delta,
\]
where $\Delta$ is the effective unit profile above. The physical equilibrium is $H^{-1}\mathbf1$. The interval is nonempty in every dimension.  A
...
crossing satisfies $\max(\chi_D,\chi_H)>\sqrt{8(m-2)}$, exponent $1/2$ is
globally optimal. No constant-optimal or complete Pareto-frontier claim is made.
