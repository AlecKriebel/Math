# Exact Diffusion Design for Maximally Collective Stable Turing Patterns

This repository contains the flagship paper and exact reproducibility package for

**Exact Diffusion Design for Maximally Collective Stable Turing Patterns in Binary-Complex Mass-Action Networks**  
*Topology-Wide All-Spectrum Localization and Exponent-Optimal Stationary Heterogeneity Trade-Offs*

The paper proves a topology-wide all-spectrum localization theorem, an exact stationary diffusion law, sharp topology-specific stationary heterogeneity bounds, an improved stable unit-equilibrium design, and an exact stable diffusion--equilibrium trade-off family.

## Versioned release

The immutable pre-submission snapshot is
[version 1.0.6](https://github.com/AlecKriebel/Math/releases/tag/maximally-collective-stable-turing-v1.0.6),
with its exact project source under the corresponding
[Git tag](https://github.com/AlecKriebel/Math/tree/maximally-collective-stable-turing-v1.0.6/maximally_collective_stable_turing_patterns_binary_complex_mass_action_networks).
All archived versions share the stable
[Zenodo concept DOI](https://doi.org/10.5281/zenodo.21753404).
Continued development remains on the
[`main` branch](https://github.com/AlecKriebel/Math/tree/main/maximally_collective_stable_turing_patterns_binary_complex_mass_action_networks).

## Main files

- `manuscript/main.pdf` — main paper;
- `manuscript/supplement.pdf` — exact technical supplement;
- `independent_verifier/` — independent exact checks;
- `public/repository/` — portable public package;
- `external_audit/` — specialist packets;
- `submission/` — author-controlled bioRxiv, arXiv, and journal bundles;
- `release/one_command_replay.sh` — full provenance replay (requires the five
  frozen lineage archives listed in `release/reproducibility.md`).

No file has been submitted or sent automatically.
