...
for m in (3,5,8,12):
    nu=m-2
    L0=1/np.sqrt(3*nu);L1=90*nu/(90*nu+1)
    ...
    ax.plot(x,8*nu/x,linestyle='--',linewidth=.9)
    ax.scatter([(23/(63*np.sqrt(3)))*91*np.sqrt(nu)],
               [np.sqrt(3*nu)*(91*nu-1)/(91*nu)],s=22)
...
ax.text(.02,.02,r'Dashed: $\chi_D\chi_H=8(m-2)$'+'\nDots: certified square-root-scaling endpoints',transform=ax.transAxes,fontsize=7.5)
