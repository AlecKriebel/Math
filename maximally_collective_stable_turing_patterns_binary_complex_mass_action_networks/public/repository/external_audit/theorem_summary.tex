...
{\normalsize Topology-wide all-spectrum localization and exponent-optimal heterogeneity trade-offs}\\[5pt]
...
\]
At $L=L_0$, both are $\Theta(\sqrt m)$.  The lower endpoint is the
boundary of the present exact modulus certificate, not a proved intrinsic
dynamical boundary. Since every crossing satisfies $\max(\chi_D,\chi_H)>\sqrt{8(m-2)}$, exponent $1/2$ is globally optimal. No constant-optimal or complete Pareto-frontier claim is made.
...
External checking should focus on: (i) exhaustion of principal SCCs, including $b=2a$; (ii) strict equality cases in the 34/35/77/84-term certificates; (iii) the principal-minor derivative monotonicity; (iv) the equilibrium-scaled fixed-mass gauge; and (v) the cubic comparison $N_m(L)>1/200$.
