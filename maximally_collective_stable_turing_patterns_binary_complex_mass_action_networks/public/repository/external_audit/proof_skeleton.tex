...
For spatial damping $tD_m$, chain elimination gives $\det(\lambda I-A_m+tD_m)=Q_mF-G$. At $(\lambda,t)=(0,1)$, the interior product telescopes to $91/90$. Retaining one exact factor yields a 77-term nonnegative boundary difference in $(x,y^2,t-1)$, strict except at $(0,0,0)$. Hence the first mode has one simple zero, all other first-mode eigenvalues are stable, and all $t>1$ modes are Hurwitz.
...
half-plane exclusion to the 84-term nonnegative certificate, strict except at
the first-mode zero.

At $t=0$, the exact homogeneous determinant, after dividing by $\det H$, is
\[
 QF_0-R,\qquad
...
The difference $(1+Ax+\tfrac54y^2)|F_0|^2-|R|^2$ has 22 nonzero
monomials.  After $U=A-1/4$, all coefficient polynomials are nonnegative and
equality is possible only at $\lambda=0$.  Also
\[
...
$6+99c+325c^2$ are positive because $c>1/8$.

The second harmonic is unchanged, while the zero harmonic changes by
$w_0(L)=w_0^{\rm ref}+\tau_m(L)\rho$. The cubic numerator is
$N_m(L)=N_m^{\rm ref}+\tau_m(L)S_m$. Exact bounds
\[
N_m^{\rm ref}>1/100,\qquad -1/10<S_m<0,\qquad \tau_m(L)<1/20
\]
were proved on the broader range $L\ge1/\sqrt{3\nu}$, which contains the
new certified interval. They give $N_m(L)>1/200$, hence $c_m(L)<0$ throughout.
...
\[
 ...
 \chi_H=\frac{\sqrt\nu}{\kappa_\nu}\frac{91\nu-1}{91\nu},
\]
so both are $\Theta(\sqrt m)$. The universal product lower bound implies any
simultaneous control must have $\max(\chi_D,\chi_H)>\sqrt{8\nu}$, so exponent
$1/2$ is optimal. Constants remain open.
