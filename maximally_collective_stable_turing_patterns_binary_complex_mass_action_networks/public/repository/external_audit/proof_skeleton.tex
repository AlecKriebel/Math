...
$D^{\rm phys}=\mathsf H_m(L)\Delta$. Under $z=\mathsf H_m(L)x$,
\[
z_t=\mathsf H_m(L)\{f(z)+\Delta z_{xx}\}.
\]
The effective critical bracket and right vector are unchanged; the dynamic
left vector is $\widetilde\ell_m(L)=\mathsf H_m(L)^{-1}\ell_m$. The physical
fixed-mass functional is $(\mathsf H_m(L)^{-1}c)^Tz$.
