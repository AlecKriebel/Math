...
reaction--diffusion systems; Turing instability; mass-action kinetics;
reaction networks; bifurcation; diffusion design.

\smallskip
\noindent\textbf{2020 Mathematics Subject Classification:}
Primary 35K57; Secondary 35B32, 37G10, 80A30, 92C42.
...
nonconstant Neumann mode on a fixed interval.  We do not claim intrinsic
finite-wavelength selection under enlargement of the spatial domain.
...
Let $I$ be a principal species set with $0<|I|<m$.  Every strongly connected
component of the induced Jacobian graph on $I$ is either a singleton, whose
corresponding diagonal entry is negative, or one of the following
non-singleton blocks:
\begin{enumerate}[leftmargin=2em]
\item the complete long cycle $X_1,\ldots,X_{m-1}$;
\item the complete long cycle $X_2,\ldots,X_m$;
\item a non-singleton principal subgraph of the boundary triad
$\{X_1,X_m,Z\}$.
\end{enumerate}
...
every other non-singleton strongly connected component is a principal subgraph
...
parameter-dependent arrow $X_1\to X_m$ disappears.  This arrow belongs to
neither complete long cycle, so both long-cycle blocks are unchanged.  Within
the boundary graph, deleting an arrow can only split strongly connected
components and cannot merge them, so no new case arises.
\end{proof}
...
Finally, the explicit lower-bidiagonal Schur-complement elimination in
Supplement, Section~S2, gives
...
calculation in Supplement~S2 gives \eqref{eq:coreminor}, hence
\eqref{eq:omitZ}.  Now omit an interior species $X_j$, $2\le j\le m-1$.
A Frobenius permutation orders the right chain fragment
$X_{j+1},\ldots,X_{m-1}$, then $\{X_1,X_m,Z\}$, then the left fragment
$X_2,\ldots,X_{j-1}$; empty fragments are omitted.  The resulting matrix is
block lower triangular.  The $m-3$ retained
...
Assume $(a,b,H)\in\mathfrak S_m$, and let the diffusion matrix be positive
diagonal:
\[
 D=\diag(d_1,\ldots,d_m,d_Z)\succ0.
\]
A nonzero
...
\]
the identity $c^TA_m=0$ and the Neumann boundary conditions make the
fixed-mass subspaces invariant.  Let
$\mathcal L_0u=A_mu+D_m\partial_{\xi\xi}u$.  The Neumann cosine expansion
$u=\sum_{k\ge0}u_k\cos(k\xi)$ reduces $\mathcal L_0$ to the matrix blocks
\begin{equation}\label{eq:fourier-fredholm}
 \mathcal L_{0,k}=A_m-k^2D_m.
\end{equation}
For $k=0$, the fixed-mass condition is $c^Tu_0=0$, and homogeneous stability
makes $A_m:c^\perp\to c^\perp$ invertible.  At $k=1$,
\eqref{eq:selectedsimple} gives a simple kernel $\operatorname{span}\{r\}$ and
range $\ell^\perp$; every block with $k\ge2$ is invertible by the all-mode
certificate.  Moreover, for all sufficiently large $k$,
\begin{equation}\label{eq:high-mode-inverse}
 A_m-k^2D_m=-k^2D_m
 \left(I-k^{-2}D_m^{-1}A_m\right),\qquad
 \left\|(A_m-k^2D_m)^{-1}\right\|
 \le \frac{2\|D_m^{-1}\|}{k^2}.
\end{equation}
Together with the finitely many low-mode inverses, this $k^{-2}$ bound maps
the compatible $L^2$ Fourier data into $H_N^2$.  Hence the range is closed and
\[
...
\]
Thus $\mathcal L_0:X_c\to Y_c$ is Fredholm of index zero, with cokernel
...
\]
Since the base equilibrium is $\1$ and, for each fixed $m$, the branch
perturbation tends to zero in $H_N^2\hookrightarrow C^0$, both
...
persists.  Indeed, at onset
\[
 \alphaS(A_m-k^2D_m)
 \le \left\|\frac{A_m+A_m^T}{2}\right\|_2-k^2d_{\min}
 \longrightarrow-\infty,
\]
so the all-mode certificate leaves a strictly positive complementary spectral
gap after only finitely many modes are checked.  Along the small branch the
linearization has fixed domain $H_N^2$ and is an analytic sectorial
perturbation of positive diagonal Neumann diffusion: the reaction Jacobian is
a bounded multiplication operator and the diffusion entries retain a positive
lower bound.  Standard elliptic coercivity and resolvent estimates therefore
prevent spectrum from entering from infinity, while Kato's stability and
analytic perturbation theories continue the finitely many isolated eigenvalues
and their spectral projections
\cite[Chapters~IV and~VII]{Kato1995}.  The Neumann diffusion generator on the
real fixed-mass $L^2$ space is sectorial with compact resolvent, and its
half-order phase space is $H^1$; in one dimension the quadratic Nemytskii map
is smooth from $H^1$ to $L^2$.  Henry's principle of linearized stability then
gives local exponential asymptotic stability in $H^1$
\cite[Chapter~5]{Henry1981}.  For
...
equilibrium concentration scales; it does not reduce the product.  Throughout
the certified interval, $\chi_D(L)>\chi_H(L)$, so
$\max(\chi_D,\chi_H)=\chi_D(L)$ is strictly increasing in $L$ and is uniquely
minimized within this family at $L=L_0$.
At $L=L_0$,
\begin{equation}\label{eq:scalingendpoint}
 ...
 \chi_H=\frac{\sqrt\nu}{\kappa_\nu}\frac{91\nu-1}{91\nu}.
\end{equation}
Thus both contrasts, and hence their maximum, are $\Theta(\sqrt m)$ at the
lower endpoint, compared with $\Theta(m)$ for the unit-equilibrium stable
profile.  Since every stationary crossing
obeys
...
The first is strictly increasing and the second strictly decreasing in $L$.
Moreover,
\[
 ...
 \quad >1.
\]
Hence $\chi_D(L)>\chi_H(L)$ throughout the interval and the maximum is
...
and, in particular, the exact infimum of $\max(\chi_D,\chi_H)$.
...
\cite{Paul2024}.  Conradi, Mincheva, and Uecker give sufficient spatial
instability conditions for a restricted mass-action class and numerically
continue nonlinear patterned branches, including numerically stable segments
in a finite example \cite{ConradiMinchevaUecker2026}.  Their work does not
supply the present all-dimensional realization-wide localization theorem or
exact contrast law.
...
a product reduction: $\chi_D(L)\chi_H(L)=\chi_D^{\rm unit}(m)$ is fixed.
Within the certified interval, $\chi_D(L)>\chi_H(L)$, and their maximum is
therefore minimized at the lower endpoint $L_0$, where it is
$\Theta(\sqrt m)$ rather than the unit-profile $\Theta(m)$.
...
\href{https://github.com/AlecKriebel/Math/tree/maximally-collective-stable-turing-v1.0.11/maximally_collective_stable_turing_patterns_binary_complex_mass_action_networks}{version 1.0.11 tagged release source tree}.
All archived versions are indexed by the stable
\href{https://doi.org/10.5281/zenodo.21753404}{Zenodo concept DOI}; the exact
preceding version 1.0.10 snapshot is archived at
\href{https://doi.org/10.5281/zenodo.22559244}{version DOI 10.5281/zenodo.22559244}.
...
AI systems are not authors.  Central claims are supported by explicit proofs
and independent exact implementations; neither numerical agreement nor AI
confidence is treated as proof.  The author assumes responsibility for all
content.
