...
every positive realization, while an order-$(n-1)$ subsystem is unstable and
the full reaction--diffusion system admits locally stable patterns.
\end{corollary}
...
\end{equation}
Its numerator reduces to $R_m+C_m\mathfrak h_m$ with
$\mathfrak h_m=\sum_{j=1}^{m-2}K_j^{-1}$.  The exact shifted-polynomial
...
$\widetilde\ell_m(L)=\mathsf H_m(L)^{-1}\ell_m$, and the physical fixed-mass vector becomes
$\mathsf H_m(L)^{-1}c$.
...
$\widetilde\ell_m(L)=\mathsf H_m(L)^{-1}\ell_m$ and the fixed-mass covector
...
result instead gives one exact infinite topology family with a topology-wide
