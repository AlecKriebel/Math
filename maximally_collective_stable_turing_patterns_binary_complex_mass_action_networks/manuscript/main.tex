...
\hypersetup{
 ...
 pdfsubject={Topology-wide all-spectrum localization and exponent-optimal heterogeneity trade-offs},
...
\normalsize Topology-Wide All-Spectrum Localization and Exponent-Optimal Heterogeneity Trade-Offs}
...
contrast infimum, the unit-equilibrium value $8(n-3)$, and the universal
trade-off $\chi_D\chi_H>8(n-3)$.  At unit equilibrium, an exact rational design
undergoes a primary supercritical bifurcation with locally exponentially
stable positive patterns.  A second exact family redistributes heterogeneity
between diffusion and equilibrium concentrations so that both contrasts are
$\Theta(\sqrt n)$, matching the universal minimax lower bound in exponent.
The proof uses explicit flux-cone design, exhaustive strongly connected block
classification, conservation-compatible Lyapunov--Schmidt reduction, and
...
Let $I$ be a principal species set with $|I|<m$.  Every nontrivial strongly
...
with $|I|<m$ is block upper triangular with diagonal blocks of these forms.
...
nonzero.  Along the interior chain the only off-diagonal arrows are
...
\begin{corollary}[Sharpness of the general subsystem-order endpoint]\label{cor:endpoint}
The upper endpoint $n-1$ in principal unstable-subsystem localization cannot
be lowered within binary-complex classical mass action: for every $n\ge4$,
one indexed topology has every principal subsystem below order $n-1$ Hurwitz
throughout every positive realization, while an order-$(n-1)$ subsystem is
unstable and the full reaction--diffusion system admits locally stable
patterns.
\end{corollary}
...
For $I\subseteq[n]$, set $a_I=(-1)^{|I|}\det J_{I,I}$ and
...
criterion, uniqueness, and scalar simplicity.  Since
$p_D(s)=\det(D)\det(sI-D^{-1}J)$, scalar simplicity is algebraic simplicity of
the zero eigenvalue of $J-s_*D$.
...
\cref{thm:diffusionray}.  The remaining information is the complete order-$m$
omission table.
...
\begin{proof}
Combine \cref{thm:diffusionray,prop:omissions}.  A second derivation uses the left vector $c$ and the right zero vector
\[
...
\begin{equation}\label{eq:Lrange}
 ...
 L_1(\nu)=\frac{90\nu}{90\nu+1}.
\end{equation}
The lower endpoint $L_0(\nu)$ is the boundary of the present exact modulus
certificate through $\nu L^2\ge1/3$; it is not asserted to be an intrinsic
...
\begin{proof}[Proof architecture]
In normalized concentrations $z=\mathsf H_m(L)x$, the PDE is
\begin{equation}\label{eq:normalizedPDE}
 z_t=\mathsf H_m(L)\{f_m(z)+\Delta_m z_{\xi\xi}\}.
\end{equation}
The stationary bracket $f_m(z)+\Delta_mz_{\xi\xi}$ and its critical right
vector are unchanged.  The dynamic left vector is
$q_m(L)=\mathsf H_m(L)^{-1}\ell_m$, and the physical fixed-mass vector becomes
$\mathsf H_m(L)^{-1}c$.
...
\]
because $\nu L^2\ge1/3$.  The corresponding boundary difference has 84
...
$D_m^{\rm phys}=\mathsf H_m(L)\Delta_m$, and the change $z=\mathsf H_m(L)x$
produces the row-scaled mode operator $\mathsf H_m(L)(A_m-t\Delta_m)$.
The left critical vector becomes $q_m(L)=\mathsf H_m(L)^{-1}\ell_m$ and the
fixed-mass covector becomes $\mathsf H_m(L)^{-1}c$.  The second harmonic is
...
Supplement and regenerated by \texttt{python verify\_symbolic\_certificates.py}.  Meanwhile
\[
...
product $23(91(m-2)-1)/63$.  The marked point is the certified square-root-scaling
...
universal cost is \eqref{eq:productlower}.  This is a complementary,
...
diffusion and equilibrium contrasts can be controlled simultaneously at the globally optimal square-root
exponent.  The remaining constant gap is explicit rather than
