...
def check_certificate_tables(certificates, tables):
    ...
    for table_index, (cert, table) in enumerate(zip(certificates, tables)):
        ...
        for term, (powers, coefficients, expression) in zip(cert['terms'], table):
            ...
            if 'coefficient' in term:
                ...
                key = 'coefficient_in_U_ascending' if table_index == 2 else 'coefficient_in_A_ascending'
...
def main():
    ...
    old_tables = parse_tables(old_tex, False)
    ...
    if changes:
        raise ValueError(f'Expected unchanged rendering, observed {len(changes)} changes')
    ...
    (HERE / 'SHIPPED_COEFFICIENT_RESULTS.json').write_text(json.dumps(result, indent=2) + '\n')
