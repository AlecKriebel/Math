\begin{abstract}
Stochastic reaction networks model interacting populations through random
reaction events.  Weak reversibility is widely expected to prevent such
systems from escaping indefinitely, but this was not known even for
second-order networks with a connected reaction graph unless every species
appeared in a special pure-species complex.  We remove that additional
restriction.  For all positive reaction rates, every closed component of the
state space has finite mean return times and a unique stationary probability
distribution; the construction also rules out explosion within each such
component.
\end{abstract}
...
\citet{AndersonKurtz2015}.  For a nonexplosive irreducible component, a basic
stability question is whether it has a stationary probability distribution,
or equivalently whether its states have finite mean return times.
...
since 2022 \citep{Agazzi2022Talk,Agazzi2025Talk}, but no manuscript for that result was publicly
available at the time of writing.  It allows a broader network structure in
two species, whereas the present result allows arbitrarily many species under
the binary, one-linkage assumptions.  The May 2026 revision of
\citet{Xu2026} still records the general bimolecular positive-recurrence
problem as open.
...
Figure~\ref{fig:architecture} summarizes the proof architecture and locates
the sole structural use of bimolecularity.
...
marked reaction target & residual factorial potential & target-following episode\\[0.35em]
\multicolumn{3}{c}{\(\Downarrow\)}\\[-0.15em]
\multicolumn{3}{c}{bimolecular top-complex alternative
\(\Longrightarrow\) rare terminal source \(\Longrightarrow\) uniform negative drift}
\end{tabular}
\caption{The proof mechanism.  The top-complex alternative either supplies
the rare terminal source shown here or produces a stoichiometric invariant
that precludes the divergent sequence.  This schematic was generated during
the disclosed AI-assisted manuscript preparation; it is expository and adds
no claim beyond the surrounding proof outline.}
...
Absorbing singleton classes are positive recurrent by convention.  For a
...
communicating class is nonexplosive and positive recurrent.
\end{theorem}
...
\item If every complex has \(q_J=1\), then
      \(M_J=\sum_{i\in J}X_i\) is a nonnegative invariant.  It diverges along
...
the bounded index \(N\wedge\sigma_K\).  All terms are integrable: after at
most \(N\) episodes, each population coordinate is bounded above by its
initial value plus \(2LN\), by the deterministic overshoot bound following
\eqref{eq:episode-drift}; the finite target set then bounds \(V\).  Conditional
expectation and telescoping therefore give
...
from irreducibility.  Explicitly, for the positive return time \(T_{x_*}^+\)
...
\begin{equation}\label{eq:occupation-formula}
 ...
 \qquad y\in\Gamma,
\end{equation}
is the normalized return-cycle occupation measure and is stationary.
...
\url{https://github.com/AlecKriebel/Math/tree/bimolecular-positive-recurrence-v0.3/bimolecular_positive_recurrence}.
...
manuscript preparation.  OpenAI ChatGPT, model GPT-5.6 Pro, was accessed
through the ChatGPT interface at \url{https://chatgpt.com} on 5--6 August 2026
for exploratory proof search, attempted counterexample construction,
verification-code development and testing, literature assistance, drafting,
and editorial criticism.  Anthropic Claude, model Opus 5 with Max effort, was
accessed through \url{https://claude.ai} on 6 August 2026 for an adversarial
review of the proof and manuscript.  Anthropic Claude Code using Opus 5 was
accessed through the Claude Code command-line application on 6 August 2026 to
publish the manuscript on the project website and perform related site
formatting; it did not supply an additional mathematical argument.

OpenAI Codex desktop was accessed on 8--9 August 2026 for a further
publication-readiness adversarial review, manuscript and verification-package
repair, and validation.  It used an OpenAI GPT-5-family coding/research model;
the exact backend deployment identifier was not exposed in the session.  All
of these systems are proprietary services of OpenAI or Anthropic and were used
without model modification or fine-tuning.  AI-generated literature leads
were not treated as scholarly authority; the release identifies the public or
scholarly sources on which its literature claims rely.

The work was developed through an iterative, human-directed research process.
The author determined the scope and claims released here, curated the
resulting package, and assumes responsibility for the submitted manuscript and
public verification materials.  The proof and exact artifacts are supplied to
make the AI-mediated research independently checkable; no claim of prior
independent expert human verification is made.  No AI system is listed as an
author.
...
contains fixed-seed adversarial stress tests.  The supplement documents the
test cases, normalization rules, and reproduction command.  These finite
checks are regression tests and adversarial calibration; they are not used in
place of the universal proof, in particular Lemma~\ref{lem:top}.
