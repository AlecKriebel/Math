...
The manifest covers every durable file under ``bimolecular_positive_recurrence``
except the manifest itself. Regenerable interpreter, test, packaging, and LaTeX
scratch files are ignored. Verification fails on missing, changed, or unexpected
durable files, so stale partial manifests cannot silently pass.
"""
...
def ignored(path: Path) -> bool:
    ...
    if relative == Path("supplement/MANIFEST.sha256"):
        return True
