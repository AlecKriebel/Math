...
class TopComplexTests(unittest.TestCase):
    ...
    def test_validator_rejects_K_mass_outside_divergent_set(self):
        complexes = [(1, 0, 0), (1, 0, 1)]
        weights = (Fraction(1), Fraction(0), Fraction(0))
        false_certificate = TopClassification(
            "K_mass_invariant",
            ((1, 0, 0), (1, 0, 1)),
            frozenset({1}),
        )
        with self.assertRaisesRegex(AssertionError, "nondivergent"):
            validate_top_classification(
                complexes,
                weights,
                frozenset({0}),
