# Remaining mathematical obligations — source revision 0.1.0

No Lean theorem in this delivery is certified. The distinction below is between
**proof bodies that have been written** and **mathematics that still has to be
implemented**, before considering compiler/API repair.
...
All present source needs real Lean elaboration and, where required, proof/API
repair. Benchmark the finite-check shards and the small rank witness using the
ordinary kernel path. No Lean time or memory estimate is supplied.
