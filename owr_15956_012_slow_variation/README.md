...
- [Zenodo upload kit](https://aleckriebel.github.io/Math/papers/odd-part-slow-variation/zenodo-upload-kit.zip) and [copy-and-paste instructions](zenodo/UPLOAD.md)
...
To rebuild the PDF, use `tectonic manuscript/paper.tex --outdir output` (or another LaTeX engine with the listed packages). To rebuild the archives and publication mirror after checks, run `python3 build_package.py`. This also creates `zenodo/zenodo-upload-kit.zip`, which is available from the download link above but is not nested inside the source archive. The builder updates only this effort's directory and its dedicated `docs/papers/odd-part-slow-variation/` mirror. The PDF build is mathematically reproducible; PDF binary identity can vary by TeX version and build date. Archives are deterministic for fixed input bytes.
