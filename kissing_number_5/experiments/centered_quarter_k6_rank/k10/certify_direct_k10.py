#!/usr/bin/env python3
"""Exactly reconstruct the discovered direct rank-five K10 mixture."""
...
def main() -> None:
    ...
    for line in lines[1:]:
        ...
        assert len(fields) == 165
        rows.append((fields[:45], fields[45:]))
    ...
    target_all = [Q(1)] + [Q(value) / 13 for value in source["nu"]]
    floating = np.array(
    ...
    certificate = {
        "schema": "kissing5.centered_quarter_direct_k10_triangle_extension.v1",
        "status": (
            "exact symmetric local rank-five Gram-PSD K10 triangle-marginal "
            "extension; not a code and not a ten-point Lasserre certificate"
        ),
        "source_certificate": str(source_path.relative_to(ROOT)),
        "source_sha256": hashlib.sha256(source_path.read_bytes()).hexdigest(),
        "generation_k9_certificate": str(k9_path.relative_to(ROOT)),
        "generation_k9_sha256": hashlib.sha256(k9_path.read_bytes()).hexdigest(),
        "grid": source["grid"],
        "normalization": (
            "atom weights sum to 1; expected count of each triangle type "
            "among the one hundred twenty faces is nu/13, so the uniform "
        ...
        "scope_warning": (
            ...
            "marginals on local K10 atoms. It does not realize a global "
