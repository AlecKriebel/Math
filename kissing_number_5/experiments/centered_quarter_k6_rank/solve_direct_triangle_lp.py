#!/usr/bin/env python3
"""Test a discovered rank-five K6 triangle-count catalog against exact nu."""
...
def main() -> None:
    ...
    for line in lines[1:]:
        ...
        edges.append(fields[:15])
        features.append(fields[15:])
    ...
    target = np.concatenate(([1.0], nu / 78.0))
    result = linprog(
    ...
    report = {
        ...
        "warning": (
            "A feasible floating solution must be reconstructed exactly. "
            "Infeasibility applies only to this incomplete discovered catalog."
        ),
    }
