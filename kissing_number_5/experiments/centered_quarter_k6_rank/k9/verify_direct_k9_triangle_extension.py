#!/usr/bin/env python3
"""Exact verifier for the direct rank-five K9 triangle-marginal mixture."""
...
CERTIFICATE_PATH = HERE / "direct_k9_triangle_extension.json"
...
def principal_determinants(
    ...
    return {
        size: [
            ...
            for indices in itertools.combinations(range(9), size)
        ]
...
def verify() -> dict[str, object]:
    ...
    assert certificate["schema"] == (
        "kissing5.centered_quarter_direct_k9_triangle_extension.v1"
    )
    ...
    assert certificate["generation_k8_sha256"] == hashlib.sha256(
        K8_PATH.read_bytes()
    ).hexdigest()
    assert certificate["generation_k8_sha256"] == (
    ...
    edge_counts = [Q(0)] * 7
    minimum_principal = {size: None for size in range(1, 10)}
    ...
    return {
        "status": "PASS",
        "scope": (
            "symmetric local rank-five Gram-PSD K9 triangle marginal; "
            "not a code"
        ),
        ...
        "all_sixth_through_ninth_order_principal_determinants": 0,
    }
