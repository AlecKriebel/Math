#!/usr/bin/env python3
"""Enumerate rank-five K9 extensions of the 51 explicit K8 atoms.

Discovery only.  The stored K8 atoms are used as rank-five bases for exact
Schur/range generation; the resulting mixture may change the K8 marginal.
"""
...
def scaled_gram8(edges: tuple[int, ...]) -> list[list[int]]:
    matrix = [[4 if i == j else 0 for j in range(8)] for i in range(8)]
    for (i, j), color in zip(PAIRS8, edges, strict=True):
...
def triangle_feature(
    ...
    for i, j, k in itertools.combinations(range(9), 3):
        colors = tuple(
            sorted(
                (
                    edges[PAIR_INDEX9[(i, j)]],
                    edges[PAIR_INDEX9[(i, k)]],
                    edges[PAIR_INDEX9[(j, k)]],
                )
            )
        )
...
def main() -> None:
    ...
    for atom_index, atom in enumerate(k8["atoms"]):
        base_edges = tuple(atom[EDGE_KEY8])
        gram = scaled_gram8(base_edges)
        choice = None
        for indices in itertools.combinations(range(8), 5):
            base = [[gram[i][j] for j in indices] for i in indices]
            base_determinant = determinant(base)
            if base_determinant > 0:
                omitted = tuple(index for index in range(8) if index not in indices)
        ...
        for color_vector in itertools.product(range(7), repeat=5):
            ...
            new_colors: list[int | None] = [None] * 8
            ...
            for index, color in zip(omitted, forced_colors, strict=True):
                new_colors[index] = color
            edges = [0] * 36
            for pair, color in zip(PAIRS8, base_edges, strict=True):
                edges[PAIR_INDEX9[pair]] = color
            for index, color in enumerate(new_colors):
                assert color is not None
                edges[PAIR_INDEX9[(index, 8)]] = color
