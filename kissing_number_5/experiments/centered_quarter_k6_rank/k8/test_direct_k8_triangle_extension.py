...
PATH = HERE / "verify_direct_k8_triangle_extension.py"
SPEC = importlib.util.spec_from_file_location("verify_direct_k8", PATH)
assert SPEC is not None and SPEC.loader is not None
VERIFY = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(VERIFY)


class DirectK8TriangleExtensionTests(unittest.TestCase):
    ...
    def test_exact_extension(self) -> None:
        ...
        self.assertEqual(
            result[
                "all_sixth_through_eighth_order_principal_determinants"
            ],
            0,
        )
