#!/usr/bin/env python3
"""Exact verifier for the direct rank-five K8 triangle-marginal mixture."""
...
CERTIFICATE_PATH = HERE / "direct_k8_triangle_extension.json"
...
def verify() -> dict[str, object]:
    ...
    assert certificate["schema"] == (
        "kissing5.centered_quarter_direct_k8_triangle_extension.v1"
    )
    ...
    assert certificate["generation_k7_sha256"] == hashlib.sha256(
        K7_PATH.read_bytes()
    ).hexdigest()
    assert certificate["generation_k7_sha256"] == (
    ...
    edge_counts = [Q(0)] * 7
    minimum_principal = {size: None for size in range(1, 9)}
    ...
    return {
        "status": "PASS",
        "scope": (
            "symmetric local rank-five Gram-PSD K8 triangle marginal; "
            "not a code"
        ),
        ...
        "all_sixth_through_eighth_order_principal_determinants": 0,
    }
