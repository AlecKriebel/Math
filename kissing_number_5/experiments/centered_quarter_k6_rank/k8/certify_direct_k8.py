#!/usr/bin/env python3
"""Exactly reconstruct the discovered direct rank-five K8 mixture."""
...
def main() -> None:
    ...
    for line in lines[1:]:
        ...
        rows.append((fields[:28], fields[28:]))
    ...
    target_all = [Q(1)] + [
        Q(7) * Q(value) / 195 for value in source["nu"]
    ]
    floating = np.array(
    ...
    certificate = {
        "schema": "kissing5.centered_quarter_direct_k8_triangle_extension.v1",
        "status": (
            "exact symmetric local rank-five Gram-PSD K8 triangle-marginal "
            "extension; not a code and not an eight-point Lasserre certificate"
        ),
        "source_certificate": str(source_path.relative_to(ROOT)),
        "source_sha256": hashlib.sha256(source_path.read_bytes()).hexdigest(),
        "generation_k7_certificate": str(k7_path.relative_to(ROOT)),
        "generation_k7_sha256": hashlib.sha256(k7_path.read_bytes()).hexdigest(),
        "grid": source["grid"],
        "normalization": (
            "atom weights sum to 1; expected count of each triangle type "
            "among the fifty-six faces is 7*nu/195, so the uniform "
        ...
        "scope_warning": (
            ...
            "marginals on local K8 atoms. It does not realize a global "
