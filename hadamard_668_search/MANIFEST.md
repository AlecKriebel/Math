...
| `verify_five_comb_paired_lobes.py` | Checks the paired-lobe theorems, octet inventories, and word-independent projective quotient |
| `verify_five_comb_core0_obstruction.py` | Checks the universal core-zero row-square obstruction, removed high-lag rows, and paired-lobe root profiles |
| `test_five_comb_core0_obstruction.py` | Focused complete regression for the core-zero verifier |
| `verify_five_comb_root12_sieve.py` | Checks the roots `+1,-1` arbitrary-placement relaxation and vertical-pair sieve |
...
| `check_lp333_sextic_quotient.py` | Checks the sextic quotient, short-word catalogs, full 333-cell skeleton, and two subfamily obstructions |
| `verify_lp333_sextic_c3.py` | Checks the residual decimation, 972-word normalization orbit, invariant signature shards, and exact C3 Burnside quotient |
| `test_lp333_sextic_quotient.py` | Focused independent sextic-quotient regressions |
| `search_lp333_sextic_cp_sat.py` | Exact 108-sign sextic quotient constructor with full LP333/H668 save gate |
| `test_lp333_sextic_cp_sat.py` | Weighted-XOR model and candidate-replay regressions |
| `run_lp333_sextic_signature_shards.py` | Historical resume runner for the now-closed 298 sextic signature shards; do not resume |
| `verify_lp333_multiplier_row_sum.py` | Reconstructs the transition identity and exact order-18/order-9/order-6 obstruction |
| `test_lp333_multiplier_row_sum.py` | Focused regression for all zero cores and the order-three boundary |
| `enumerate_lp333_order3_row_sums.cpp` | Exhaustively emits the 1,756 exact order-three row-sum PAF words |
| `output/lp333_order3_row_sum_catalog.csv` | Byte-pinned complete order-three row-sum catalog |
| `verify_lp333_order3_difference_family.py` | Replays the 24-triple lift, all 333 entries, and exact non-candidate residuals |
| `test_lp333_order3_difference_family.py` | Focused pure-axis and mixed-residual regressions |
| `verify_lp333_order3_mod3_sieve.py` | Replays the ten-state factorization, Eisenstein equivalence, 22 shards, and 3,334/10,000 sieve |
...
| `verify_lp333_order3_profile_endpoint_shell.py` | Exhaustively excludes the `n_9=6` profile-energy endpoint |
| `test_lp333_order3_profile_endpoint_shell.py` | Focused endpoint-shell census, symmetry, and exact-replay regressions |
| `verify_lp333_order3_profile_penultimate_shell.py` | Exhaustively excludes the `n_9=5` shell by an affine modulo-nine join |
...
| `conference_334_z37_lift/verify_z37_lift_frontier.py` | Reconstructs the seed quotient, lift obstructions, exact characteristic-2/3 censuses, trace and moment laws, formal integration, and companion rank-two closures |
...
| `conference_334_z37_lift/census_z37_quotients.cpp` | Assertion-enabled orderly enumeration of all 625 integral quotient classes |
| `conference_334_z37_lift/audit_z37_quotient_parity.cpp` | Independent reduction of the integral census to three parity types and two complement types |
| `conference_334_z37_lift/audit_z37_all_quotient_ambient.py` | Exact 625-class trace-law and first-moment ambient census |
...
| `verify_lp333_phase_cone_trivial_branch.py` | Checks the period-three margin obstruction and exact nonzero catalog image |
| `test_lp333_phase_cone_trivial_branch.py` | Focused kernel, lift, catalog, and projective-ratio regressions |
| `verify_lp333_order3_phase_trace_sieve.py` | Checks row-Galois/trace inversion, Parseval cuts, physical idempotence, and local norm cubics |
| `test_lp333_order3_phase_trace_sieve.py` | Focused full-algebra, decoder, alphabet, and negative-cone regressions |
| `verify_lp333_order3_phase_cyclic_decoder.py` | Checks the mod-seven split, factor orientation, local alphabets, and exact architecture bounds |
| `test_lp333_order3_phase_cyclic_decoder.py` | Focused finite-field, compatibility, barrier, and certificate regressions |
...
| `test_lp333_order3_lift_catalog.py` | Focused histogram-model and coverage regressions |
| `verify_lp333_order3_quotient.py` | Dependency-free transition, symmetry, full-correlation, and H(668) candidate gate |
| `search_lp333_order3_cp_sat.py` | Exact 216-sign order-three quotient constructor with row-sum channel |
| `test_lp333_order3_cp_sat.py` | Dedicated quotient-model, symmetry, and candidate-rejection regressions |
| `check_bs84_sidelnikov_fold.py` | Checks the Sidelnikov identity, direct/product exclusions, and orientation fingerprints |
| `search_bs84_sidelnikov_decimations.cpp` | Exact full-signature join for independently decimated character blocks |
| `verify_bs84_oriented_sds.py` | Strict prime-fold, adjacent-fold lift, and full `668 x 668` verifier |
| `test_bs84_oriented_sds.py` | Prime-fold verifier and retained-checkpoint regression suite |
| `output/bs84_oriented_sds_local_p19.json` | Verified nonexact profile-19 checkpoint, quarter-energy 14 |
