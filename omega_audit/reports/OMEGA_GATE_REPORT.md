...
The core Jacobian has shape `14 x 10`; at the strict source point its first
six orbit rows and columns `(x0,x1,x2,x3,x4,x7)` have determinant
`-723/8589934592`.  This readable lower witness and the complete row reduction
are regenerated by `omega_audit/independent/verify_omega_rank_readability.py`.
