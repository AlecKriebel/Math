...
\(H_n(3,6)\)~\cite{Wenzl1988}.  Explicitly, in the normalization used for
this quotient,
\[
  \eta_{6,3}
  ...
  \qquad q=e^{i\pi/3}.
\]
This is the identification used in \cite[Theorem 5.28]{GHR2013}.
...
The known operator in~\cite[Eq.~(5.2), Theorem~5.28, and
Remark~5.29(1)]{GHR2013} is instead of type
...
The public source package contains three supported verification routes and the
unaltered original attachment from which one route was hardened.
\begin{enumerate}[leftmargin=2em]
  \item \texttt{verify\_supplied.py} is a hardened SymPy implementation of
  the original attachment.  It uses explicit, optimization-safe checks and
  reconstructs the matrices and main identities exactly.  The byte-for-byte
  original is retained separately as \texttt{verify\_supplied\_original.py}
  for provenance and is not a supported execution path.
  ...
  the \(64\times64\) Yang--Baxter equation, both partial traces, both
  dimension-three obstruction norms, the active Hecke relation, the literal
  printed active operator, the \(32\times32\) generalized
  equation~\eqref{eq:32gybe}, and far commutativity.
...
All comparisons are exact; floating-point tolerances are not used.  These
finite computations verify the displayed matrix, word, trace, and obstruction
identities.  Tower faithfulness, the dimension-three classification step, and
the priority conclusion remain mathematical or literature arguments rather
than finite computational checks.
...
development, literature support, and manuscript preparation.  Every claim in
the theorem is supported by the printed derivation, and the finite identities
and trace calculations are checked by the exact programs described above.
No mathematical conclusion depends on accepting an AI-generated assertion or
on reproducing the original numerical search.
...
and the localization and unitary-braided-vector-space literature provide
close prior art~\cite{Rowell2011,RowellWang2012,GalindoRowell2014}.  Lechner then
...
A focused search through 16 August 2026 of primary sources, scholarly indexes,
general web results,
...
The curated submission package supplies SHA-256 checksums and explicit
manuscript and code licenses.  The maintained
...
or analyzed; the exact code, frozen outputs, and sources reproduce all
computational claims and accompany this manuscript in a dedicated archival
record whose DOI is cited in the submitted edition.
...
research and manuscript-preparation process}

OpenAI ChatGPT 5.6 Sol Pro through chatgpt.com and OpenAI ChatGPT 5.6 Sol
Ultra through the ChatGPT desktop application were used for research, review,
and composition.  Anthropic Claude Opus 5 Max through claude.ai was used for
review.  Uses included mathematical exploration and derivation, adversarial
proof analysis, symbolic and software work, literature support, and manuscript
preparation.
The work was developed through an iterative, human-directed research process.
The author determined the scope and claims released here, curated the resulting
package, reviewed and edited the content, and assumes responsibility for the
submitted manuscript and public verification materials.  The author takes
full responsibility for every claim released here.  The proof and exact
artifacts are supplied to make the AI-mediated research independently
checkable.
...
\newblock \url{https://arxiv.org/abs/2603.20158v1}.
