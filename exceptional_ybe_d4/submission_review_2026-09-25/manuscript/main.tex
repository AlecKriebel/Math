...
\hypersetup{
  ...
  citecolor=deepblue,
  urlcolor=deepblue,
  pdftitle={A five-word Pauli--Clifford normal form for an exceptional unitary Hecke Yang--Baxter operator},
...
\title{\vspace{-1.4cm}\textbf{A five-word Pauli--Clifford normal form}\\[0.2em]
\textbf{for an exceptional unitary}\\[0.2em]
\textbf{Hecke Yang--Baxter operator}}
\author{Alec Kriebel\\[-0.1em]
\small Independent researcher, San Francisco, USA\\[-0.1em]
\small Corresponding author: \href{mailto:me@aleckriebel.com}{me@aleckriebel.com}
\quad\textperiodcentered\quad
\href{https://orcid.org/0009-0001-9320-500X}{ORCID 0009-0001-9320-500X}}
\date{Submission revision --- 25 September 2026}

\begin{document}
\maketitle

\begin{abstract}
We give a five-word real Pauli--Clifford normal form for an exceptional unitary
Hecke Yang--Baxter operator on \(\bbC^4\otimes\bbC^4\), with eigenvalues
\(-1,e^{i\pi/3}\) of equal multiplicity.  Four words combine into an involution
\(M\), while a fifth involution \(E\) anticommutes with \(M\).  A complete
\(18\)-word cubic certificate shows that, on the reflection circle
\(\alpha M+\beta E\), the associated Hecke operator satisfies the braid relation exactly when
\(\alpha^2=2/3\) and \(\beta^2=1/3\).  Scalar partial traces of the spectral
projection give a direct Markov-trace proof that the tensor-space
representations faithfully localize the Jones--Wenzl tower \(H_n(3,6)\) for
every strand number.  A dimension-three obstruction, using the known
classification of two-eigenvalue braid characters, combines with the known
dimension-two obstruction to prove that local dimension four is minimal.
We also exhibit an active three-qubit generalized operator with shift two.
Galindo and Rowell independently obtained the same exceptional local solution
through a broader quaternionic construction.  We give an exact local-unitary
comparison with the opposite of their operator and extend it to an
equivalence on every braid word.  This identifies the known finite-image and
Clifford structures in the five-word coordinates.  A direct scalar enhancement
identifies its matrix-trace link invariant with twice the normalized HOMFLYPT
polynomial at \((i,i)\).  Exact verification programs accompany the proofs.
\end{abstract}

\medskip
\noindent\textbf{Keywords.}
Yang--Baxter operator; Hecke algebra; braid group representation; unitary
localization; Pauli--Clifford normal form; link invariant.
...
possible counterexample to the Rowell--Wang localization conjecture.  Lechner isolated
...
\medskip
\noindent\textbf{Independent concurrent work.}
...
on 28 July 2026.\footnote{The public release is available as
\href{https://github.com/AlecKriebel/Math/releases/tag/exceptional-ybe-d4-v1.1.0}
{\texttt{exceptional-ybe-d4-v1.1.0}}; the accompanying provenance record
gives the timestamp and artifact identifiers.}
...
The distinguishing contribution is the sparse normal form and its short,
checkable proof architecture: the complete cubic certificate, a direct
partial-trace localization argument, the dimension-three obstruction, and
the exact local and all-strand comparisons.  The finite-image and classical
link-evaluation results are consequences of established quaternionic results,
with their normalizations made explicit here.  In particular, the paper does
not claim a new braid-image family or a new link invariant.
...
the six-letter word basis.  Exactly the \(18\) words in
Table~\ref{tab:certificate} survive:
\begin{center}
\small
\captionof{table}{Complete word expansion of the cubic residual before
imposing \(\alpha^2+\beta^2=1\).}
...
\[
  ...
  \qquad q=e^{i\pi/3}.
\]
This is the Wenzl Markov parameter for \(H_n(3,6)\)
\cite[Theorem~3.6(b), p.~379]{Wenzl1988}.  GHR \(\S5.6\) identifies
...
The preprint of GHR prints \(q=e^{2\pi i/3}\) at the start of \(\S5.6\),
whereas the braid eigenvalues and
...
\cite{RowellWang2012}, as restated in
\cite{GHR2013}, holds for the simple tensor generator \(X\) of
...
The placement here differs from the \((3,1)\)-generalized localization in
\cite[Theorem~5.28 and Remark~5.29(1)]{GHR2013}.  For an
...
\end{aligned}
\]
For the present operator, direct exact calculation gives, with
\(\|\cdot\|_F\) the Frobenius norm,
\[
  \|\mathcal Y_1(K)\|_F^2=24,\qquad \mathcal Y_2(K)=0.
\]
Thus the displayed \(K\) does not also satisfy the shift-one equation.
The shift-two equation is what permits the spectator blocking
\(\Id_2\otimes K\) into an ordinary operator on the local space
\(\bbC^2\otimes\bbC^2\).  No equivalence or inequivalence with the older
generalized matrix is needed for this argument.
...
eight-dimensional generalized operators discussed above, write
\(R_{\mathrm K}:=R\).
...
\begin{equation}
  ...
  \label{eq:RGR}
\end{equation}
These are the ordinary-operator formulas in
\cite[\S13]{GalindoRowell2026}.
...
to opposite and local basis change.

\section{Finite braid images and the enhanced link invariant}
\label{sec:global}

All links considered here are nonempty and oriented.
...
a standard computational Clifford operator is needed.

For completeness, finiteness follows directly from this normalization and a
determinant bound.  Include the scalar phases \(\{\pm1,\pm i\}\) in
\(\mathcal P_{2n}\), so this is a finite group.  Conjugation maps
\(G_n=\rho_n^{\mathrm K}(B_n)\) into the finite automorphism group of its
conjugated Pauli frame.  The frame spans the full matrix algebra, so the
kernel consists of scalar matrices.  Since
\[
  \det R=(-1)^8q^8=\kappa,\qquad
  \det\rho_n^{\mathrm K}(\sigma_i)
  =\kappa^{4^{n-2}}=\kappa,
\]
every element of \(G_n\) has determinant in the group of cube roots of unity.
A scalar \(\lambda\Id_{4^n}\in G_n\) therefore satisfies
\(\lambda^{3\cdot4^n}=1\).  The kernel as well as the image of the conjugation
action is finite, proving finiteness of \(G_n\).
This is a direct proof for the displayed normalization of the established
quaternionic finite-image phenomenon
\cite[Theorem~3.1]{Rowell2011}, also covered by
\cite[Theorem~8.4]{GalindoRowell2026}.
\end{proof}
...
For orientation and crossing conventions, Table~\ref{tab:links} gives small
checks from the exact matrix trace.
\begin{center}
\captionof{table}{Values of the enhanced invariant on oriented braid closures.}
\label{tab:links}
\(
...
Online Resource~1 supplies five supported exact-verification routes:
...
image, the generalized operator and far commutativity, the shift-one
residual of \(K\), and the finite identities
...
README records the commands and scope.  These are five checking routes,
not five independent arithmetic implementations: the two custom field
implementations share formulas, and the braid-and-link route imports
arithmetic and matrix helpers from the concurrent-comparison route.
The SymPy route and the matrix-free word certificate supply complementary
checks.  These finite checks do not replace the
...
in its classification setting.  The distinct proof architecture and
certificate of this paper are summarized in the Introduction.
Proposition~\ref{prop:concurrent-comparison}
supplies an exact bridge to their Family~III formula, and
Theorem~\ref{thm:all-strand} extends it to every braid word.

The public and reported private chronology is recorded in
...
uniqueness.  The circle certificate is complete only within its specified
two-parameter family.  ``Five-word'' describes the displayed support; no
minimum-support theorem over all local changes of basis is claimed.
The comparison theorem concerns the \(16\times16\) ordinary
Family~III operator \(R_{\mathrm{GR}}\); it makes no comparison theorem about
the older \(8\times8\) generalized localization.

\section*{Statements and Declarations}

\subsection*{Data and code availability}

The exact verification programs, dependency specifications, scientific
failure-mode tests, frozen outputs, checksums, and a statement of their scope
accompany this submission as Online Resource~1.  The source for this
submission revision accompanies the manuscript separately.  The preceding
version 1.2.0 and its original verification package remain archived at
\href{https://doi.org/10.5281/zenodo.22013710}
{doi:10.5281/zenodo.22013710}~\cite{Kriebel2026Zenodo}.
Version 1.1.3 remains archived at
\href{https://doi.org/10.5281/zenodo.21971507}
{doi:10.5281/zenodo.21971507}.  Both archives are unchanged.
The \href{https://aleckriebel.github.io/Math/papers/exceptional-ybe-d4/}
{project page} provides public manuscript and verification links.  No
...
and composition, and Anthropic Claude through claude.ai for review.
OpenAI Codex was also used for the submission revision, including separately
tasked adversarial reviews and development of additional exact checks.
These AI-assisted reviews are not independent human peer review or a formal
verification of the entire paper.  The author takes responsibility for the
content of the publication.

\subsection*{Supplementary information}

Online Resource~1: Exact verification code, scientific failure-mode tests,
reproducible outputs, and coverage notes for the five-word operator and its
local and global comparisons (\texttt{ESM\_1.zip}).
...
\newblock Preprint: \url{https://arxiv.org/abs/1105.5048v1}.
...
\newblock \url{https://doi.org/10.1063/1.4880196}.
...
\newblock \url{https://doi.org/10.48550/arXiv.2608.16865};
...
\newblock \url{https://doi.org/10.5281/zenodo.22013710}.
...
\newblock \url{https://doi.org/10.1007/BF02621920}.
...
\newblock \url{https://doi.org/10.48550/arXiv.2603.20158};
...
\newblock \url{https://doi.org/10.4171/QT/18}.
