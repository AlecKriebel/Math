...
This package accompanies Version 1.2 of *Positive Recurrence for Single-Linkage Bimolecular Weakly Reversible Stochastic Reaction Networks*.

## Reproduce the committed report

Python 3.11 or newer is required. The verifier uses only the Python standard
library; no installation, package manager, network access, or third-party
runtime dependency is needed. The supplied runner is a POSIX shell script; on
native Windows, use WSL or reproduce its displayed Python commands.
...
- exact lifted population-state return cycles, including the zero complex,
  boundary states, multiple linkage classes, parallel channels, and identical
  population displacements, together with finite reachability-symmetry checks;
- the corrected fixed-state rate-degeneration limit and its exact large-state
  logarithmic coefficient;
- the unshifted entropy drift and complete marked-target episode for Anderson,
  Cappelletti, and Kim's Example 4.1, compared by exact rational
  prime-exponent signatures;
...
- explicit boundary-face, absorbing-singleton, finite-class, parity,
  ...
  Foster, two- and three-state regenerative occupation, and rate-degeneration
  calibrations.
...
`verification_report.json` is canonical JSON with sorted keys and fixed
separators. The committed report has been verified byte-for-byte under CPython
3.11, 3.12, 3.13, and 3.14. No claim is made for untested Python
implementations or future minor versions. Any future divergence in the
fixed-seed stress-test digest is a reproducibility failure, not evidence about
the analytic theorem. The report excludes operating-system names, timestamps,
elapsed times, temporary paths, and generated transcripts. Its source hashes
come from the closed allowlist `SOURCE_FILES` in
`src/bimolecular_pr/verification.py`; an unlisted Python source or test causes
...
`CITATION.cff`. The maintained Version 1.2 source is in the
[Math repository](https://github.com/AlecKriebel/Math/tree/bimolecular-positive-recurrence-v1.2/bimolecular_positive_recurrence_submission_v1_2/code).
