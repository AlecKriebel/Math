# Version 1.2.2 reproduction record

**Release date:** 16 August 2026
**Canonical tag:** `bimolecular-positive-recurrence-v1.2.2`
...
```bash
git rev-parse bimolecular-positive-recurrence-v1.2.2^{commit}
```
...
cd Math
git checkout --detach bimolecular-positive-recurrence-v1.2.2
bimolecular_positive_recurrence_submission_v1_2_2/validation/replay_release.sh
```
...
The Version 1.2.1 tag, archive, manifests, PDFs, and hosted replay remain
unchanged. This patch release carries the verifier component byte-for-byte and
changes one current literature-status sentence, its bibliography note,
submission instructions, and outer release records only.
