...
permanently toward ever-larger populations.  We prove that, for every initial
population and every choice of positive reaction rates, its reachable states
form a closed class.  Every nonabsorbing such class is positive recurrent,
while an absorbing singleton carries its point-mass law, when the network is
weakly reversible, has one linkage class, and no reaction complex contains
more than two molecules.  Thus every initial population has a unique
stationary probability law on its reachable class.  This removes an additional
pure-species condition required by the previous theorem.  The proof follows
the target produced by each reaction and measures the population remaining
after that target is subtracted.  Multiple linkage classes and reactions
involving more than two molecules remain open.
\end{abstract}
\else
\begin{abstract}
Weak reversibility is conjectured to imply positive recurrence for stochastic
mass-action systems.  We prove the conjecture for bimolecular networks with
one linkage class, removing the earlier requirement that every species occur
in a pure unary or pure-double complex.  For every positive rate vector and
every initial population, weak reversibility makes the reachable set a closed
...
result as of 10 August 2026.  The public descriptions concern two species and
...
\]
At \(x_n=(n,1,0)\), their unshifted entropy-like function has generator
drift \(\kappa_1n(2\log2-1)\to+\infty\).  For the marked construction,
...
gives uniqueness.  For an absorbing singleton, the unique stationary law is
the point mass, as stated in Theorem~\ref{thm:main}.
...
\href{https://github.com/AlecKriebel/Math/tree/bimolecular-positive-recurrence-v1.1/bimolecular_positive_recurrence_submission_v1_1}{the tagged V1.1 repository directory}.
The submission archive is also supplied with the manuscript so that its exact
Git snapshot and checksums can be retained with the submission record.  No
computation is used in the universal proof.  The dependency-free verifier
checks exact algebraic identities, the scalar envelope, deterministic finite
state-cycle calibrations, the rate-degeneration limit, the ACK example, and
finite stationary-cycle normalization.  No finite atlas or random test proves
recurrence, \(K\) is not enumerated, and the universal theorem is analytic;
details are in the reviewer supplement.
...
Generative-AI systems were used materially from 5--10 August 2026 for
mathematical exploration and derivation, counterexample search, symbolic
computation and code, literature support, drafting, and adversarial review.  The
tools were OpenAI ChatGPT with GPT-5.6 Pro through the web interface;
Anthropic Claude with Opus 5 at Max effort through claude.ai; Anthropic Claude
Code with Opus 5 through the command-line application;
and OpenAI Codex desktop with a GPT-5-family backend whose exact identifier was
not exposed.  Rejected AI-assisted approaches are not part of the final proof.
The author directed the research, determined the released scope and claims,
curated the package, and assumes responsibility for the manuscript and public
verification materials.  No independent expert human validation is claimed,
no AI system is an author, and the full tool-by-tool statement is
\href{https://github.com/AlecKriebel/Math/blob/bimolecular-positive-recurrence-v1.1/bimolecular_positive_recurrence_submission_v1_1/supplement/ai_use_full_statement.md}{supplement/ai\_use\_full\_statement.md}.
