...
## 2026-09-24T04:13:30.252759+00:00 — Published Zenodo record audit (100% of record check; DOI resolution unresolved)

The user supplied DOI 10.5281/zenodo.22929857. The public Zenodo record is version 1.1 with the correct title, author, ORCID, preprint type, publication date, open access, license, scoped description and related links. Both deposited files match the reviewed local files byte for byte; Zenodo MD5 checksums and ZIP integrity also pass. Creator affiliation is omitted, an optional metadata difference.

The DOI resolver returns HTTP 404 and visibly displays DOI NOT FOUND, while the direct Zenodo record and downloads work. The record assigns the expected identifier, so a registration/activation delay is possible; this has not been established or repaired. Use the direct record link for current sharing. Updated live citation and project/site deposit notices, preserving the frozen archives. Detailed receipt: research/zenodo_22929857_audit.json. No comment or email was sent. The user specifically authorized drafting a comment and notification email; these are returned to the user for their own use.
