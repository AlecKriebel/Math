\section{The rank-one no-promotion pair
selector}

\subsection{1. Exact split and claim
boundary}
...
\item
  ...
  1.1 of \emph{rank\_\texttt{\seqsplit{one\_corrected\_factorial\_endpoint.md}}};
\item
...
\item
  on all-active passing descriptors the fixed linear correction has
  bounded jump increments and does not change the Anderson--Kim tier
  descent.
\end{itemize}
...
also checks the stronger fact needed here: each of the 310 rank-one
pairs has one common whole-top mask across all its rank-one descriptors.

Thus the 233-pair statement is a certified
\textbf{dimension-at-least-two common-potential theorem}. It is not
automatically a recurrence theorem because 92 of the 233 pairs still
have an affine-feasible one-active failed descriptor. The recently
refuted universal one-active old-debt theorem is not used here.

\subsubsection{1.1 Discrete transfer on the 154 reversible all-active
pairs}

The all-active Proposition 5.2 of
\emph{three\_\texttt{\seqsplit{active\_shell\_gluing\_gate.md}}} is written for the
continuous rate-adjusted entropy
...
There is an exact discrete proof which is stronger than a formal
Stirling comparison. Write \(z=y+\zeta\), let \(\alpha=\kappa_{yz}\),
\(\beta=\kappa_{zy}\), and put
...
(2.2) of \emph{rank\_\texttt{\seqsplit{one\_corrected\_factorial\_endpoint.md}}}, and
\(\log u\le u-1\) give
...
polynomial of degree at most two. Its positive part is a finite sum of
the changed-coordinate curvature-cofactor monomials (with a bounded
boundary remainder). Thus Proposition 5.2's cofactor hypothesis makes
(1.4c) \(O(\beta_n)\) directly. The forced lower-linkage exit, evaluated
with the exact factorial jump identity, has drift divided by \(\beta_n\)
tending to minus infinity. This already proves the discrete all-active
theorem.

\begin{quote}
\textbf{Lemma 1.1 (Stirling finite-difference transfer).} Let
...
\(\alpha_n=x_n^y\asymp x_n^z\), and let \(\beta_n=\max_{u\in R}x_n^u\)
for the lower linkage. By (1.4),
...
makes (1.8) \(O(\beta_n)\). Every lower propensity is at most order
\(\beta_n\), and (1.4) is bounded globally, so
...
Equivalently, the continuous-entropy proof supplies the forced lower
exit
...
while \({\cal L}_TU_\theta=O(\beta_n)\). Equations (1.8)--(1.10) prove
(1.5).
...
every positive lower-tier edge is still controlled by \(g e^{-g}\). Thus
replacing \(U_\theta\) by \({\cal F}_\theta\) does not change the
ordinary Anderson--Kim conclusion. This proves that the potential used
by the two-active endpoint is literally the potential used in the
all-active and passing regions.

\subsection{2. The 141-pair recurrence
theorem}
...
\subsubsection{Theorem 2.1 (pair-level common-potential
composition)}
...
\tightlist
\item
  it is a two-active failed flat sequence, and the audited physical
  episode has expected \({\cal F}_*\)-drift tending to \(-\infty\);
\item
  it is an all-active failed sequence, and the audited all-active
  generator theorem, together with Lemma 1.1 in the reversible case,
  gives \({\cal L}{\cal F}_*\to-\infty\); or
\item
  it is a passing descriptor, and the ordinary descending-source
...
set. The ten possible \emph{zero\_boundary\_phase\_only} incidences do
not create a fourth alternative in \(\Gamma\): at their displayed
inactive cap the lower linkage is disabled, the top linkage preserves
that zero coordinate and \(H_w\), and Proposition 4.1 of
\emph{two\_active\_promotion\_phase.md} confines \(\Gamma\) to one
finite top shell. Thus no divergent sequence in a fixed \(\Gamma\)
realizes such an incidence.

Lemma 3.1 of \emph{physical\_entropy\_gluing\_lemma.md} turns the
...
the discrete reversible transfer in Lemma 1.1, the directed-triple
branch, all passing descriptors, the sequence-to-finite-exception step,
endpoint and duration integrability, common-potential gluing,
nonexplosion, and the classwise positive-return conclusion. It found no
remaining gap. It also checked exact disjointness from the 151 affine,
fourteen rank-two, 51 all-active-only, and twelve \(H_b\)-seam
selectors.

Therefore every closed irreducible class of each of these 141 support
pairs is positive recurrent. The other 92 pairs in the 233-pair local
branch are not included: each retains at least one affine-feasible
one-active failed descriptor.

\subsection{3. Exact hashes and
reproduction}
...
common-potential and exact 141-pair recurrence flags. Global T3-2
remains uncertified.
