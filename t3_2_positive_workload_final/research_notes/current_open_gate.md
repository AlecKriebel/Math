...
The eighth proof uses one corrected-factorial potential on every passing
cone and on the unique failed two-active cone. The dormant
\(\{B,2A\}\) phases are contracted only at physical reaction times:
\(BC\)-source exits have priority \(N^2\), \(A/AC\) interruptions have
order \(N\), reflected workload debt is at most two, and strong connectivity
...
one-step tier sequence to twelve canonical asymptotic descriptors. This
finite statement is certified in `global_atlas_interface_closure.md` and the
accompanying source/tests. It is not itself a recurrence theorem.
...
2. a lower-priority coordinate reaches the next active scale, so the episode
   passes to a higher-dimensional descriptor with controlled endpoint cost;
3. the leading cofactor dynamics is genuinely closed and weakly reversible,
   and has quantitative hitting, occupation, and endpoint moments sufficient
   for a physical-time Foster episode; or
...
- tightness as exact finite support;
- bounded slower-event count as control of fast embedded occupation;
- reaction deletion or recurrence monotonicity under support enlargement;
- pointwise shell drift without an integrated clearing bound;
